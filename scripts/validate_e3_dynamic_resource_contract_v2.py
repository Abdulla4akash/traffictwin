...
"""Fail-closed predeclaration validator for E3 dynamic-resource v2 contract v2.
...
v2 retains all v1 invariants and adds two narrow semantic corrections:
- waiting-room occupancy drain per-RSU transition;
- comparator rejection precedence for ingress_dla and per_task_dla.
Provenance must bind exact SHAs and reasons.
"""
...
CONTRACT_VERSION = "e3_dynamic_resource_v2_contract_v2"
V1_CONTRACT_VERSION = "e3_dynamic_resource_v2_contract_v1"
BASE_COMMIT = "80e8ae55dfbcc0aa271ed7ed1d67aeae8f384761"
BRANCH = "worker/e3-lane-01-contract"
CAMPAIGN = "e3-dynamic-resource-v2"

# v2 successor provenance binding
EXPECTED_SUCCESSOR_CANDIDATE_SHA = "79abfe89f9eb55f490c9f578172256d7a13714a3"
EXPECTED_SUCCESSOR_INTEGRATION_SHA = "bbd955ed181d2a82183158ded8ad86116809bf27"
EXPECTED_V1_JSON_SHA256 = "0d8c545703405c59dce5b1d1c7d38a63b31341c23364de43c6530862931fa160"
EXPECTED_V1_CONTRACT_PATH = "docs/evaluation/e3/e3_dynamic_resource_v2_contract_v1.json"
EXPECTED_SUCCESSOR_REASONS = [
...
EXPECTED_SUCCESSOR_AUTHORING_BRANCH = "worker/e3-lane-01-contract-v3"
EXPECTED_SUCCESSOR_PREPARED_BASE_SHA = "bbd955ed181d2a82183158ded8ad86116809bf27"
EXPECTED_APPLIES_TO = ["ingress_dla", "per_task_dla"]
V2_ONLY_BLOCKS = frozenset(
    {
...
# Exact key sets for fail-closed v2-only blocks (additive keys forbidden)
EXPECTED_SUCCESSOR_PROVENANCE_KEYS = frozenset(
    {
        "exact_reasons",
        "note",
        "reasons",
        "successor_authoring_branch",
        "successor_prepared_base_sha",
        "successor_version",
        "superseded_candidate_sha",
        "superseded_promoted_integration_sha",
        "v1_contract_path",
        "v1_json_sha256",
    }
)
EXPECTED_SUCCESSOR_NOTE = (
    "v1 remains historical; v2 is the successor binding the two missing scientific definitions"
)
EXPECTED_WAITING_ROOM_KEYS = frozenset(
    {
        "active_capacity_units",
        "backlog_post",
        "drain_work_ms",
        "dynamic_capacity_changes_work_drain_and_indirectly_released_occupancy",
        "equations",
        "fixed_1x_must_reproduce_frozen_E2d_exactly",
        "floor_removal_is_tasks_drained",
        "floor_term_is_tasks_removed_not_remaining_occupancy",
        "forbidden_behaviors",
        "full_work_drain_forces_zero_occupancy",
        "lifecycle_fields_remain_null",
        "never_multiplies_waiting_room_ceiling",
        "per_rsu_once_per_1000ms_outer_tick_after_all_five_task_slots",
        "pin_vectors",
        "positive_residual_work_retains_at_least_one_task",
        "simulated_count_transition_does_not_make_lifecycle_available",
        "transition",
        "true_pre_drain_backlog_work_ms",
        "true_pre_drain_waiting_room_task_count",
        "uses_true_pre_drain_state_never_stale_view",
        "waiting_room_ceiling_never_scaled_with_compute_units",
        "waiting_room_ceiling_tasks_per_rsu",
    }
)
EXPECTED_WAITING_ROOM_EQUATIONS_KEYS = frozenset(
    {
        "B_post",
        "B_pre",
        "D",
        ...
        "L_post_when_partial",
        "L_pre",
        "tasks_drained_when_full",
        "tasks_drained_when_partial",
    }
)
EXPECTED_WAITING_ROOM_EQUATIONS = {
    "B_post": "B_pre - D",
    "B_pre": "true pre-drain backlog_work_ms",
    "D": "min(B_pre, active_capacity_units * 1000)",
    "L_post_when_full": "0",
    "L_post_when_partial": "max(L_pre - tasks_drained, 1)",
    "L_pre": "true pre-drain waiting-room task count",
    "tasks_drained_when_full": "L_pre",
    "tasks_drained_when_partial": "floor(L_pre * D / max(B_pre, 1e-6))",
}
EXPECTED_WAITING_ROOM_TRANSITION_KEYS = frozenset({"else_B_post_gt_0", "if_B_post_lte_0"})
EXPECTED_WAITING_ROOM_TRANSITION_BRANCH_KEYS = frozenset({"L_post", "condition", "tasks_drained"})
EXPECTED_WAITING_ROOM_LIFECYCLE_KEYS = frozenset(
    {
        "compute_completed",
...
EXPECTED_WAITING_ROOM_LIFECYCLE = {
...
EXPECTED_WAITING_ROOM_FORBIDDEN = [
    "treating floor(L*D/B) as L_post rather than removed count",
    "ceil or round instead of floor",
    "capacity-independent 1x drain",
    "stale occupancy as drain input",
    "failure to zero when backlog empties",
    "failure to retain one when backlog remains positive",
...
EXPECTED_COMPARATOR_KEYS = frozenset(
    {
        ...
        "p2c_separate",
        "precedence",
        "preserves_frozen_E2d_taxonomy_exactly",
    }
)
EXPECTED_COMPARATOR_CROSS_KEYS = frozenset(
    {
        ...
        "total_rejection_and_rejection_share_are_comparable_across_strategies",
    }
)
EXPECTED_COMPARATOR_DETAILS_KEYS = frozenset(
    {
        "cap_uses_true_current_plus_same_tick_not_stale",
        "execution_is_minus1_for_rejected",
        "gate_equality_is_gate_rejected",
        "radio_is_current_not_stale",
        "rejected_work_never_reserved_or_executed",
        "stale_occupancy_never_used_for_cap",
        "strict_gate_is_lt",
        "target_fullness_uses_current_true_occupancy_at_task_slot_entry",
        "target_fullness_uses_stale_free_occupancy",
        "v2i_cap_rejected_trigger",
        "v2i_gate_rejected_trigger",
        "v2i_unavailable_trigger",
    }
)
EXPECTED_V2I_CAP_REJECTED_TRIGGER = (
    "live in-slot cap failure caused by prior admitted reservations "
    "(true_current_waiting_room_occupancy[rsu] + prior same-tick reservations "
    ">= queue_ceiling for target)"
)
EXPECTED_V2I_GATE_REJECTED_TRIGGER = (
    "strict observed-backlog gate failure "
    "(observed_decision_backlog_work_ms[rsu] >= task_deadline_ms)"
)
EXPECTED_V2I_UNAVAILABLE_TRIGGER = "current radio failure OR target already full at task-slot entry"
EXPECTED_COMPARATOR_FORBIDDEN = [
    "cap precedence over simultaneous gate failure",
    "task-slot-entry fullness relabelled cap rejection",
    "radio failure relabelled gate or cap",
    "stale occupancy used for either cap check",
    "rejected work reserved or executed",
    "comparator precedence replacing P2C taxonomy",
    (
        "individual V2I rejection classes presented as identically "
        "constructed cross-strategy contrasts"
    ),
]
EXPECTED_COMPARATOR_ORDERED_STEPS = [
    "select comparator target",
    "inactive/non-V2I yields no E3 V2I rejection class",
    "current radio failure OR target already full at task-slot entry -> v2i_unavailable",
    "otherwise strict observed-backlog gate failure -> v2i_gate_rejected",
    "otherwise live in-slot cap failure caused by prior admitted reservations -> v2i_cap_rejected",
    "otherwise admit",
...
EXPECTED_COMPARATOR_PRECEDENCE_KEYS = frozenset(
    {
        "step_1_select_comparator_target",
        "step_2_inactive_non_V2I_yields_no_E3_V2I_rejection_class",
        "step_3_current_radio_failure_OR_target_already_full_at_task_slot_entry_yields_v2i_unavailable",
        "step_4_otherwise_strict_observed_backlog_gate_failure_yields_v2i_gate_rejected",
        "step_5_otherwise_live_in_slot_cap_failure_caused_by_prior_admitted_reservations_yields_v2i_cap_rejected",
        "step_6_otherwise_admit",
...
EXPECTED_EXECUTION_AUTHORITY_KEYS = frozenset(
    {
        ...
        "engineering_functionality_authorized",
        "evidence_state",
        "full_3600_step_cells_authorized",
        "hold_release_authority",
        "lane_09",
        "manchester_trace_comparative_execution_authorized",
        "minimal_construct_smoke_only",
        "multi_seed_or_fleet_draw_execution_authorized",
        "performance_evidence_benchmark_authorized",
        "planned_56_cell_design_is_dormant_predeclaration",
        "reduced_pilot_exploratory_campaign_authorized",
        "research_workloads_launched",
        "result_availability",
        "scientific_execution_authorized",
        "smoke_is_scientific_evidence",
        "software_completion_depends_on_experiment",
        "statistical_inference_authorized",
        "status",
    }
)
EXPECTED_EXECUTION_DORMANT_NOTE = (
...
def _resolve_v1_path_safely(declared_path: str, repo_root: Path) -> tuple[Path | None, str | None]:
    try:
        repo_resolved = repo_root.resolve()
        p = Path(declared_path)
        if p.is_absolute():
            return None, f"v1_contract_path must be relative not absolute: {declared_path!r}"
        candidate = (repo_resolved / p).resolve()
        try:
            candidate.relative_to(repo_resolved)
        except ValueError:
            return (
                None,
                f"v1_contract_path escapes repository: {declared_path!r} resolves to {candidate}",
            )
        return candidate, None
    except Exception as e:
        return None, f"v1 path resolution failed for {declared_path!r}: {e}"


def _hash_bytes_sha256(data: bytes) -> str:
...
def _canonical_json_hash(obj: object) -> str:
    return hashlib.sha256(
        json.dumps(
            obj, sort_keys=True, ensure_ascii=False, separators=(",", ":"), indent=None
        ).encode("utf-8")
    ).hexdigest()


def _load_v1_for_projection(
    data: dict[str, Any],
    v1_data_override: dict[str, Any] | None,
    v1_bytes_override: bytes | None,
    v1_path_override: Path | str | None,
    repo_root_override: Path | None,
    errors: list[str],
) -> dict[str, Any] | None:
    prov = (
        data.get("successor_provenance")
        if isinstance(data.get("successor_provenance"), dict)
        else {}
    )
    declared_path = prov.get("v1_contract_path") if isinstance(prov, dict) else None
    if not isinstance(declared_path, str):
        declared_path = EXPECTED_V1_CONTRACT_PATH
    if v1_data_override is not None:
        if v1_bytes_override is None:
            _err(
                errors,
                "v1_data_override supplied without v1_bytes_override: unauthenticated object override forbidden "  # noqa: E501
                "(must supply v1_bytes_override with exact expected SHA and matching parsed JSON)",
            )
            return None
        actual_hash = _hash_bytes_sha256(v1_bytes_override)
        if actual_hash != EXPECTED_V1_JSON_SHA256:
            _err(
                errors,
                f"v1 bytes hash mismatch: got {actual_hash} expected {EXPECTED_V1_JSON_SHA256} "  # noqa: E501
                "(explicit bytes override)",
            )
            return None
        try:
            parsed_override = json.loads(v1_bytes_override.decode("utf-8"))
            if parsed_override != v1_data_override:
                _err(errors, "v1_data_override does not match v1_bytes_override parsed JSON")
                return None
        except Exception as e:
            _err(errors, f"v1_bytes_override is not valid JSON: {e}")
            return None
        return v1_data_override
    if v1_bytes_override is not None:
        actual_hash = _hash_bytes_sha256(v1_bytes_override)
        if actual_hash != EXPECTED_V1_JSON_SHA256:
            _err(
                errors,
            ...
            return None
        try:
            return json.loads(v1_bytes_override.decode("utf-8"))  # type: ignore[no-any-return]
        except Exception as e:
            _err(errors, f"v1 bytes is not valid JSON: {e}")
            return None
    repo_root = repo_root_override if repo_root_override is not None else _repo_root_default()
    if v1_path_override is not None:
        if isinstance(declared_path, str) and declared_path != EXPECTED_V1_CONTRACT_PATH:
            _err(
                errors,
                f"v1_contract_path must be {EXPECTED_V1_CONTRACT_PATH!r} got {declared_path!r}",
            )
            return None
        declared_to_resolve = str(v1_path_override)
    else:
        declared_to_resolve = declared_path
        if declared_to_resolve != EXPECTED_V1_CONTRACT_PATH:
            _err(
                errors,
                (
                    f"v1_contract_path must be {EXPECTED_V1_CONTRACT_PATH!r} "
                    f"got {declared_to_resolve!r}"
                ),
            )
            return None
    candidate, err = _resolve_v1_path_safely(declared_to_resolve, repo_root)
    if err is not None:
        _err(errors, err)
        return None
    ...
    if not candidate.is_file():
        _err(
            errors,
            f"v1 contract file missing or not file: {candidate} (declared {declared_to_resolve!r})",
        )
        return None
    try:
        raw_bytes = candidate.read_bytes()
    except Exception as e:
        _err(errors, f"v1 contract file unreadable: {candidate}: {e}")
        return None
    actual_hash = _hash_bytes_sha256(raw_bytes)
    if actual_hash != EXPECTED_V1_JSON_SHA256:
        _err(
            errors,
            (
                f"v1 contract byte hash mismatch: got {actual_hash} "
                f"expected {EXPECTED_V1_JSON_SHA256} (file {candidate})"
            ),
        )
        return None
    if isinstance(prov, dict) and prov.get("v1_json_sha256") != actual_hash:
        _err(
            errors,
            (
                f"successor_provenance v1_json_sha256 "
                f"{prov.get('v1_json_sha256')!r} does not match actual "
                f"v1 byte hash {actual_hash}"
            ),
        )
        return None
    try:
        ...
        _err(errors, f"v1 contract JSON invalid: {candidate}: {e}")
        return None
    if not isinstance(parsed, dict):
        _err(errors, f"v1 contract JSON must be dict, got {type(parsed)}")
        return None
    return parsed


def _validate_inherited_projection(
    data: dict[str, Any], v1_data: dict[str, Any], errors: list[str]
) -> None:
    import copy

    proj = copy.deepcopy(data)
    for block in V2_ONLY_BLOCKS:
        proj.pop(block, None)
    if isinstance(v1_data, dict) and "schema_version" in v1_data:
        proj["schema_version"] = v1_data["schema_version"]
    else:
        proj["schema_version"] = V1_CONTRACT_VERSION
    if proj != v1_data:
        proj_hash = _canonical_json_hash(proj)
        v1_hash = _canonical_json_hash(v1_data)
        _err(
            errors,
            (
                f"inherited v1 projection hash mismatch: proj {proj_hash} "
                f"!= v1 {v1_hash} (full projection must exactly equal v1 "
                f"after removing {sorted(V2_ONLY_BLOCKS)} and normalizing "
                "schema_version)"
            ),
        )
        v1_keys = set(v1_data.keys()) if isinstance(v1_data, dict) else set()
        proj_keys = set(proj.keys()) if isinstance(proj, dict) else set()
        extra = proj_keys - v1_keys
        missing = v1_keys - proj_keys
        if extra:
            _err(
                errors, f"inherited projection has extra top-level keys not in v1: {sorted(extra)}"
            )
        if missing:
            _err(errors, f"inherited projection missing v1 top-level keys: {sorted(missing)}")
        for k in sorted(v1_keys & proj_keys):
            if proj[k] != v1_data[k]:
                pv = json.dumps(proj[k], sort_keys=True)[:500]
                vv = json.dumps(v1_data[k], sort_keys=True)[:500]
                _err(errors, f"inherited projection key {k!r} drift: proj {pv!r} != v1 {vv!r}")
                break
    else:
        if _canonical_json_hash(proj) != _canonical_json_hash(v1_data):
            _err(
                errors,
                (
                    "inherited projection hash mismatch even though dict "
                    "equality passed (deterministic hash must match)"
                ),
            )
...
def validate_contract(
    ...
    # Successor provenance — binds exact superseded SHAs and reasons
    prov = data.get("successor_provenance")
    if not isinstance(prov, dict):
        _err(errors, "successor_provenance missing or not dict (v2 provenance required)")
    else:
        _check_exact_keys(prov, EXPECTED_SUCCESSOR_PROVENANCE_KEYS, "successor_provenance", errors)
        if prov.get("note") != EXPECTED_SUCCESSOR_NOTE:
            _err(
                errors,
                f"successor_provenance note must be exactly {EXPECTED_SUCCESSOR_NOTE!r} "
                f"got {prov.get('note')!r}",
            )
        if prov.get("superseded_candidate_sha") != EXPECTED_SUCCESSOR_CANDIDATE_SHA:
            _err(
                errors,
                f"successor_provenance superseded_candidate_sha must be "
                f"{EXPECTED_SUCCESSOR_CANDIDATE_SHA}",
            )
        if prov.get("superseded_promoted_integration_sha") != EXPECTED_SUCCESSOR_INTEGRATION_SHA:
            _err(
                errors,
                f"successor_provenance superseded_promoted_integration_sha must be "
                f"{EXPECTED_SUCCESSOR_INTEGRATION_SHA}",
            )
        if prov.get("v1_json_sha256") != EXPECTED_V1_JSON_SHA256:
            _err(
                errors,
                f"successor_provenance v1_json_sha256 must be {EXPECTED_V1_JSON_SHA256}",
            )
        reasons = prov.get("reasons")
        if reasons != EXPECTED_SUCCESSOR_REASONS:
            _err(
                errors,
                (
                    f"successor_provenance reasons must be exactly "
                    f"{EXPECTED_SUCCESSOR_REASONS!r} in declared order with "
                    f"no extra/duplicate/reorder, got {reasons!r}"
                ),
            )
        else:
            # Additional duplicate/extra check is covered by exact equality,
            # but also ensure no duplicate via set
            if len(reasons) != len(set(reasons)):
                _err(
                    errors, f"successor_provenance reasons must have no duplicates, got {reasons!r}"
                )
        exact = prov.get("exact_reasons")
        if exact != EXPECTED_EXACT_REASONS:
            _err(
                errors,
                (
                    f"successor_provenance exact_reasons must be exactly "
                    f"{EXPECTED_EXACT_REASONS!r}, got {exact!r}"
                ),
            )
        if prov.get("successor_version") != "v2":
            _err(errors, "successor_provenance successor_version must be v2")
        if prov.get("v1_contract_path") != EXPECTED_V1_CONTRACT_PATH:
            _err(
                errors,
                f"successor_provenance v1_contract_path must be {EXPECTED_V1_CONTRACT_PATH!r}",
            )
        if prov.get("successor_authoring_branch") != EXPECTED_SUCCESSOR_AUTHORING_BRANCH:
            _err(
                errors,
                (
                    f"successor_provenance successor_authoring_branch must be "
                    f"{EXPECTED_SUCCESSOR_AUTHORING_BRANCH!r}, got "
                    f"{prov.get('successor_authoring_branch')!r}"
                ),
            )
        if prov.get("successor_prepared_base_sha") != EXPECTED_SUCCESSOR_PREPARED_BASE_SHA:
            _err(
                errors,
                (
                    f"successor_provenance successor_prepared_base_sha must be "
                    f"{EXPECTED_SUCCESSOR_PREPARED_BASE_SHA!r}, got "
                    f"{prov.get('successor_prepared_base_sha')!r}"
                ),
            )
    ...
    # v2 narrow corrections — waiting-room occupancy drain
    occ = data.get("waiting_room_occupancy_transition")
    if not isinstance(occ, dict):
        _err(
            errors,
            "waiting_room_occupancy_transition missing or not dict (v2 occupancy correction "
            "required)",
        )
        occ = {}
    else:
        _check_exact_keys(
            occ, EXPECTED_WAITING_ROOM_KEYS, "waiting_room_occupancy_transition", errors
        )
        if occ.get("per_rsu_once_per_1000ms_outer_tick_after_all_five_task_slots") is not True:
            _err(
                errors,
                "waiting_room_occupancy_transition "
                "per_rsu_once_per_1000ms_outer_tick_after_all_five_task_slots "
        ...
        if occ.get("uses_true_pre_drain_state_never_stale_view") is not True:
            _err(
                errors,
                "waiting_room_occupancy_transition uses_true_pre_drain_state_never_stale_view must "
                "be "
                "true",
            )
        if occ.get("true_pre_drain_backlog_work_ms") != "B_pre":
            _err(
                errors,
                "waiting_room_occupancy_transition true_pre_drain_backlog_work_ms must be 'B_pre'",
            )
        if occ.get("true_pre_drain_waiting_room_task_count") != "L_pre":
            _err(
                errors,
                (
                    "waiting_room_occupancy_transition "
                    "true_pre_drain_waiting_room_task_count must be 'L_pre'"
                ),
            )
        if occ.get("active_capacity_units") != "u":
            _err(
                errors,
                "waiting_room_occupancy_transition active_capacity_units must be 'u'",
            )
        if occ.get("drain_work_ms") != "D = min(B_pre, active_capacity_units * 1000)":
            _err(
                errors,
                "waiting_room_occupancy_transition drain_work_ms must be D = min(B_pre, "
                "active_capacity_units "
                "* 1000)",
            )
        if occ.get("backlog_post") != "B_post = B_pre - D":
            _err(
                errors, "waiting_room_occupancy_transition backlog_post must be B_post = B_pre - D"
            )
        trans = occ.get("transition", {})
        if not isinstance(trans, dict):
            _err(errors, "waiting_room_occupancy_transition transition missing or not dict")
        else:
            _check_exact_keys(
                trans,
                EXPECTED_WAITING_ROOM_TRANSITION_KEYS,
                "waiting_room_occupancy_transition.transition",
                errors,
            )
            if_full = trans.get("if_B_post_lte_0", {})
            if isinstance(if_full, dict):
                _check_exact_keys(
                    if_full,
                    EXPECTED_WAITING_ROOM_TRANSITION_BRANCH_KEYS,
                    "waiting_room_occupancy_transition.transition.if_B_post_lte_0",
                    errors,
                )
            else_partial_check = trans.get("else_B_post_gt_0", {})
            if isinstance(else_partial_check, dict):
                _check_exact_keys(
                    else_partial_check,
                    EXPECTED_WAITING_ROOM_TRANSITION_BRANCH_KEYS,
                    "waiting_room_occupancy_transition.transition.else_B_post_gt_0",
                    errors,
                )
            if (
            ...
            if (
                not isinstance(else_partial, dict)
                or else_partial.get("tasks_drained") != "floor(L_pre * D / max(B_pre, 1e-6))"
                or else_partial.get("L_post") != "max(L_pre - tasks_drained, 1)"
                or else_partial.get("condition") != "B_post > 0"
            ):
                _err(
                    errors,
                    "waiting_room_occupancy_transition else_B_post_gt_0 must be "
                    "floor(L_pre*D/max(B_pre,1e-6)) "
                    "and max(L_pre-tasks_drained,1) condition B_post > 0",
                )
        if occ.get("floor_term_is_tasks_removed_not_remaining_occupancy") is not True:
            _err(
                errors,
                "waiting_room_occupancy_transition "
                "floor_term_is_tasks_removed_not_remaining_occupancy "
        ...
        if occ.get("floor_removal_is_tasks_drained") is not True:
            _err(
                errors,
                (
                    "waiting_room_occupancy_transition "
                    "floor_removal_is_tasks_drained must be true (floor "
                    "removal is tasks drained, not remaining occupancy)"
                ),
            )
        if (
            occ.get("dynamic_capacity_changes_work_drain_and_indirectly_released_occupancy")
            is not True
        ):
            _err(
                errors,
        ...
        if occ.get("never_multiplies_waiting_room_ceiling") is not True:
            _err(
                errors,
        ...
        if occ.get("full_work_drain_forces_zero_occupancy") is not True:
            _err(
                errors,
                "waiting_room_occupancy_transition full_work_drain_forces_zero_occupancy must be "
                "true",
            )
        if occ.get("positive_residual_work_retains_at_least_one_task") is not True:
            _err(
                errors,
                "waiting_room_occupancy_transition "
                "positive_residual_work_retains_at_least_one_task "
        ...
        if occ.get("fixed_1x_must_reproduce_frozen_E2d_exactly") is not True:
            _err(
                errors,
                "waiting_room_occupancy_transition fixed_1x_must_reproduce_frozen_E2d_exactly must "
                "be "
                "true",
            )
        if occ.get("simulated_count_transition_does_not_make_lifecycle_available") is not True:
            _err(
                errors,
                "waiting_room_occupancy_transition "
                "simulated_count_transition_does_not_make_lifecycle_available "
        ...
        equations = occ.get("equations", {})
        if not isinstance(equations, dict):
            _err(errors, "waiting_room_occupancy_transition equations missing or not dict")
        else:
            _check_exact_keys(
                equations,
                EXPECTED_WAITING_ROOM_EQUATIONS_KEYS,
                "waiting_room_occupancy_transition.equations",
                errors,
            )
            for _ek, _ev in EXPECTED_WAITING_ROOM_EQUATIONS.items():
                if equations.get(_ek) != _ev:
                    _err(
                        errors,
                        f"waiting_room_occupancy_transition equations {_ek} must be {_ev!r} "
                        f"got {equations.get(_ek)!r}",
                    )
        pin = occ.get("pin_vectors")
        if not isinstance(pin, list) or len(pin) != 3:
            ...
            for idx, vec in enumerate(pin):
                if not isinstance(vec, dict):
                    continue
                B_pre = vec.get("B_pre")  # noqa: N806
                L_pre = vec.get("L_pre")  # noqa: N806
                u = vec.get("u")
                D = vec.get("D")  # noqa: N806
                B_post = vec.get("B_post")  # noqa: N806
                td = vec.get("tasks_drained")
                Lp = vec.get("L_post")  # noqa: N806
                if (
                    isinstance(B_pre, (int, float))
                    and isinstance(u, (int, float))
                    and isinstance(D, (int, float))
                ):
                    exp_D = min(B_pre, u * 1000)  # noqa: N806
                    if exp_D != D:
                        _err(
                            errors,
                            f"waiting_room_occupancy_transition pin_vectors[{idx}] D must be "
                            f"min(B_pre,u*1000) got {D} expected {exp_D}",
                        )
                if (
                    isinstance(B_pre, (int, float))
                    and isinstance(D, (int, float))
                    and isinstance(B_post, (int, float))
                ):
                    exp_Bpost = B_pre - D  # noqa: N806
                    if B_post != exp_Bpost:
                        _err(
                            errors,
                            f"waiting_room_occupancy_transition pin_vectors[{idx}] B_post must be "
                            f"B_pre-D got {B_post} expected {exp_Bpost}",
                        )
                if isinstance(B_post, (int, float)) and isinstance(L_pre, (int, float)):
                    if B_post <= 0:
                        if td != L_pre or Lp != 0:
                            _err(
                                errors,
                                f"waiting_room_occupancy_transition pin_vectors[{idx}] full drain "
                                f"must have tasks_drained=L_pre L_post=0",
                            )
                    else:
                        # partial: floor(L*D / max(B,1e-6)) and max(L - floor,1)
                        if (
                            isinstance(B_pre, (int, float))
                            and isinstance(D, (int, float))
                            and isinstance(L_pre, (int, float))
                        ):
                            import math as _math

                            exp_td = _math.floor(L_pre * D / max(B_pre, 1e-6))
                            exp_Lp = max(L_pre - exp_td, 1)  # noqa: N806
                            if td != exp_td:
                                _err(
                                    errors,
                                    f"waiting_room_occupancy_transition pin_vectors[{idx}] "
                                    f"tasks_drained {td} must equal floor(L*D/B) {exp_td}",
                                )
                            if Lp != exp_Lp:
                                _err(
                                    errors,
                                    f"waiting_room_occupancy_transition pin_vectors[{idx}] L_post "
                                    f"{Lp} must equal max(L-tasks_drained,1) {exp_Lp}",
                                )
        lifecycle = occ.get("lifecycle_fields_remain_null", {})
        if not isinstance(lifecycle, dict):
            _err(
                errors,
                "waiting_room_occupancy_transition "
                "lifecycle_fields_remain_null missing or not dict",
            )
        else:
            _check_exact_keys(
                lifecycle,
                EXPECTED_WAITING_ROOM_LIFECYCLE_KEYS,
                "waiting_room_occupancy_transition.lifecycle_fields_remain_null",
                errors,
            )
            for _lk, _lv in EXPECTED_WAITING_ROOM_LIFECYCLE.items():
                if lifecycle.get(_lk) != _lv:
                    _err(
                        errors,
                        f"waiting_room_occupancy_transition lifecycle_fields_remain_null {_lk} "
                        f"must be {_lv!r} got {lifecycle.get(_lk)!r}",
                    )
        fb = occ.get("forbidden_behaviors")
        if fb != EXPECTED_WAITING_ROOM_FORBIDDEN:
            _err(
                errors,
                f"waiting_room_occupancy_transition forbidden_behaviors must be exactly "
                f"{EXPECTED_WAITING_ROOM_FORBIDDEN!r} ordered, got {fb!r}",
            )

    # v2 narrow corrections — comparator rejection precedence
    comp = data.get("comparator_rejection_precedence")
    if not isinstance(comp, dict):
        _err(
            errors,
            "comparator_rejection_precedence missing or not dict (v2 comparator correction "
            "required)",
        )
        comp = {}
    else:
        _check_exact_keys(comp, EXPECTED_COMPARATOR_KEYS, "comparator_rejection_precedence", errors)
        applies = comp.get("applies_to")
        if applies != EXPECTED_APPLIES_TO:
            _err(
                errors,
                (
                    f"comparator_rejection_precedence applies_to must be "
                    f"exactly {EXPECTED_APPLIES_TO!r} ordered with no "
                    f"extra/duplicate/reorder, got {applies!r}"
                ),
            )
        else:
            if len(applies) != len(set(applies)):
                _err(
                    errors,
                    (
                        f"comparator_rejection_precedence applies_to must "
                        f"have no duplicates, got {applies!r}"
                    ),
                )
        if comp.get("preserves_frozen_E2d_taxonomy_exactly") is not True:
            _err(
                errors,
                "comparator_rejection_precedence preserves_frozen_E2d_taxonomy_exactly must be "
                "true",
            )
        if comp.get("p2c_approved_n0_taxonomy_remains_separate_and_unchanged") is not True:
            _err(
                errors,
                "comparator_rejection_precedence "
                "p2c_approved_n0_taxonomy_remains_separate_and_unchanged "
        ...
        prec = comp.get("precedence", {})
        if not isinstance(prec, dict):
            _err(errors, "comparator_rejection_precedence precedence missing or not dict")
        else:
            _check_exact_keys(
                prec,
                EXPECTED_COMPARATOR_PRECEDENCE_KEYS,
                "comparator_rejection_precedence.precedence",
                errors,
            )
            for k in sorted(EXPECTED_COMPARATOR_PRECEDENCE_KEYS):
                if prec.get(k) is not True:
                    _err(errors, f"comparator_rejection_precedence precedence {k} must be true")
        ordered_steps_val: Any = comp.get("ordered_steps")
        if ordered_steps_val != EXPECTED_COMPARATOR_ORDERED_STEPS:
            _err(
                errors,
                "comparator_rejection_precedence ordered_steps must be "
                f"exactly {EXPECTED_COMPARATOR_ORDERED_STEPS!r} ordered, got {ordered_steps_val!r}",
            )
        details = comp.get("details", {})
        if not isinstance(details, dict):
            _err(errors, "comparator_rejection_precedence details missing or not dict")
        else:
            _check_exact_keys(
                details,
                EXPECTED_COMPARATOR_DETAILS_KEYS,
                "comparator_rejection_precedence.details",
                errors,
            )
            if details.get("strict_gate_is_lt") is not True:
                _err(
                    errors, "comparator_rejection_precedence details strict_gate_is_lt must be true"
                )
            if details.get("gate_equality_is_gate_rejected") is not True:
                _err(
                    errors,
                    "comparator_rejection_precedence details gate_equality_is_gate_rejected must "
                    "be "
                    "true",
                )
            if details.get("cap_uses_true_current_plus_same_tick_not_stale") is not True:
                _err(
                    errors,
                    "comparator_rejection_precedence details "
                    "cap_uses_true_current_plus_same_tick_not_stale "
            ...
            if details.get("stale_occupancy_never_used_for_cap") is not True:
                _err(
                    errors,
                    "comparator_rejection_precedence details stale_occupancy_never_used_for_cap "
                    "must "
                    "be true",
                )
            if details.get("rejected_work_never_reserved_or_executed") is not True:
                _err(
                    errors,
                    "comparator_rejection_precedence details "
                    "rejected_work_never_reserved_or_executed "
            ...
            if details.get("execution_is_minus1_for_rejected") is not True:
                _err(
                    errors,
                    "comparator_rejection_precedence details execution_is_minus1_for_rejected must "
                    "be "
                    "true",
                )
            if details.get("radio_is_current_not_stale") is not True:
                _err(
                    errors,
                    "comparator_rejection_precedence details radio_is_current_not_stale must be "
                    "true",
                )
            if details.get("target_fullness_uses_stale_free_occupancy") is not True:
                _err(
                    errors,
                    "comparator_rejection_precedence details "
                    "target_fullness_uses_stale_free_occupancy must be true "
                    "(current true task-slot-entry fullness, never stale occupancy)",
                )
            if (
                details.get("target_fullness_uses_current_true_occupancy_at_task_slot_entry")
                is not True
            ):
                _err(
                    errors,
                    "comparator_rejection_precedence details "
                    "target_fullness_uses_current_true_occupancy_at_task_slot_entry "
                    "must be true (current true task-slot-entry fullness, "
                    "never stale occupancy)",
                )
            if details.get("v2i_unavailable_trigger") != EXPECTED_V2I_UNAVAILABLE_TRIGGER:
                _err(
                    errors,
                    f"comparator_rejection_precedence details v2i_unavailable_trigger must be "
                    f"exactly {EXPECTED_V2I_UNAVAILABLE_TRIGGER!r} got "
                    f"{details.get('v2i_unavailable_trigger')!r}",
                )
            if details.get("v2i_gate_rejected_trigger") != EXPECTED_V2I_GATE_REJECTED_TRIGGER:
                _err(
                    errors,
                    f"comparator_rejection_precedence details v2i_gate_rejected_trigger must be "
                    f"exactly {EXPECTED_V2I_GATE_REJECTED_TRIGGER!r} got "
                    f"{details.get('v2i_gate_rejected_trigger')!r}",
                )
            if details.get("v2i_cap_rejected_trigger") != EXPECTED_V2I_CAP_REJECTED_TRIGGER:
                _err(
                    errors,
                    f"comparator_rejection_precedence details v2i_cap_rejected_trigger must be "
                    f"exactly {EXPECTED_V2I_CAP_REJECTED_TRIGGER!r} got "
                    f"{details.get('v2i_cap_rejected_trigger')!r}",
                )
        cross = comp.get("cross_strategy_comparability", {})
        if not isinstance(cross, dict):
            _err(
                errors,
                "comparator_rejection_precedence cross_strategy_comparability missing or not dict",
            )
        else:
            _check_exact_keys(
                cross,
                EXPECTED_COMPARATOR_CROSS_KEYS,
                "comparator_rejection_precedence.cross_strategy_comparability",
                errors,
            )
            if (
                cross.get("total_rejection_and_rejection_share_are_comparable_across_strategies")
                is not True
            ):
                _err(
                    errors,
            ...
            if cross.get("individual_V2I_rejection_classes_are_mechanism_diagnostics") is not True:
                _err(
                    errors,
                    "comparator_rejection_precedence cross "
                    "individual_V2I_rejection_classes_are_mechanism_diagnostics "
        ...
        p2c_sep = comp.get("p2c_separate", {})
        if not isinstance(p2c_sep, dict):
            _err(errors, "comparator_rejection_precedence p2c_separate missing or not dict")
        else:
            _check_exact_keys(
                p2c_sep,
                EXPECTED_COMPARATOR_P2C_SEPARATE_KEYS,
                "comparator_rejection_precedence.p2c_separate",
                errors,
            )
            if p2c_sep.get("p2c_n0_taxonomy_unchanged") is not True:
                _err(
                    errors,
                    "comparator_rejection_precedence p2c_separate "
                    "p2c_n0_taxonomy_unchanged must be true",
                )
            if p2c_sep.get("comparator_precedence_does_not_replace_p2c_taxonomy") is not True:
                _err(
                    errors,
                    "comparator_rejection_precedence p2c_separate "
                    "comparator_precedence_does_not_replace_p2c_taxonomy must be true",
                )
            if (
                p2c_sep.get(
                    "p2c_uses_v2i_unavailable_v2i_gate_rejected_v2i_cap_rejected_for_n0_only"
                ...
                _err(
                    errors,
                    "comparator_rejection_precedence p2c_separate "
                    "p2c_uses_v2i_unavailable_v2i_gate_rejected_"
                    "v2i_cap_rejected_for_n0_only must be true",
                )
        fb2 = comp.get("forbidden_behaviors")
        if fb2 != EXPECTED_COMPARATOR_FORBIDDEN:
            _err(
                errors,
                f"comparator_rejection_precedence forbidden_behaviors must be exactly "
                f"{EXPECTED_COMPARATOR_FORBIDDEN!r} ordered, got {fb2!r}",
            )
    # v2 execution authority — HARD E3 scientific-execution hold (researcher-imposed)
    ea = data.get("execution_authority")
    if not isinstance(ea, dict):
        _err(
            errors,
            "execution_authority missing or not dict (v2 execution hold required)",
        )
        ea = {}
    else:
        _check_exact_keys(ea, EXPECTED_EXECUTION_AUTHORITY_KEYS, "execution_authority", errors)
        # Exact status
        if ea.get("status") != "E3_SCIENTIFIC_EXECUTION_NOT_AUTHORIZED":
            _err(
                errors,
                "execution_authority status must be E3_SCIENTIFIC_EXECUTION_NOT_AUTHORIZED",
            )
        if ea.get("scientific_execution_authorized") is not False:
            _err(
                errors,
                "execution_authority scientific_execution_authorized must be false",
            )
        if ea.get("lane_09") != "BLOCKED_BY_RESEARCHER_EXECUTION_HOLD":
            _err(
                errors,
                "execution_authority lane_09 must be BLOCKED_BY_RESEARCHER_EXECUTION_HOLD",
            )
        if ea.get("engineering_functionality_authorized") is not True:
            _err(
                errors,
                "execution_authority engineering_functionality_authorized must be true",
            )
        if ea.get("minimal_construct_smoke_only") is not True:
            _err(
                errors,
                "execution_authority minimal_construct_smoke_only must be true",
            )
        if ea.get("manchester_trace_comparative_execution_authorized") is not False:
            _err(
                errors,
                (
                    "execution_authority "
                    "manchester_trace_comparative_execution_authorized must "
                    "be false"
                ),
            )
        if ea.get("full_3600_step_cells_authorized") is not False:
            _err(
                errors,
                "execution_authority full_3600_step_cells_authorized must be false",
            )
        if ea.get("e3a_authorized") is not False:
            _err(errors, "execution_authority e3a_authorized must be false")
        if ea.get("e3b_authorized") is not False:
            _err(errors, "execution_authority e3b_authorized must be false")
        if ea.get("e3c_authorized") is not False:
            _err(errors, "execution_authority e3c_authorized must be false")
        if ea.get("reduced_pilot_exploratory_campaign_authorized") is not False:
            _err(
                errors,
                "execution_authority reduced_pilot_exploratory_campaign_authorized must be false",
            )
        if ea.get("multi_seed_or_fleet_draw_execution_authorized") is not False:
            _err(
                errors,
                "execution_authority multi_seed_or_fleet_draw_execution_authorized must be false",
            )
        if ea.get("performance_evidence_benchmark_authorized") is not False:
            _err(
                errors,
                "execution_authority performance_evidence_benchmark_authorized must be false",
            )
        if ea.get("empirical_e3_results_authorized") is not False:
            _err(
                errors,
                "execution_authority empirical_e3_results_authorized must be false",
            )
        if ea.get("statistical_inference_authorized") is not False:
            _err(
                errors,
                "execution_authority statistical_inference_authorized must be false",
            )
        _rwl = ea.get("research_workloads_launched")
        if type(_rwl) is not int or _rwl != 0:
            _err(
                errors,
                f"execution_authority research_workloads_launched must be exactly int 0 "
                f"(type is int, got {type(_rwl).__name__} {_rwl!r})",
            )
        if ea.get("evidence_state") != "NOT_EXECUTED":
            _err(errors, "execution_authority evidence_state must be NOT_EXECUTED")
        if ea.get("result_availability") != "NO_E3_RESEARCH_RESULTS_AVAILABLE":
            _err(
                errors,
                "execution_authority result_availability must be NO_E3_RESEARCH_RESULTS_AVAILABLE",
            )
        if ea.get("smoke_is_scientific_evidence") is not False:
            _err(
                errors,
                "execution_authority smoke_is_scientific_evidence must be false",
            )
        if ea.get("software_completion_depends_on_experiment") is not False:
            _err(
                errors,
                "execution_authority software_completion_depends_on_experiment must be false",
            )
        if ea.get("hold_release_authority") != "future_explicit_researcher_instruction_only":
            _err(
                errors,
                (
                    "execution_authority hold_release_authority must be "
                    "future_explicit_researcher_instruction_only"
                ),
            )
        if ea.get("planned_56_cell_design_is_dormant_predeclaration") is not True:
            _err(
                errors,
                "execution_authority planned_56_cell_design_is_dormant_predeclaration must be true",
            )
        dormant_note: Any = ea.get("dormant_predeclaration_note")
        if dormant_note != EXPECTED_EXECUTION_DORMANT_NOTE:
            _err(
                errors,
                f"execution_authority dormant_predeclaration_note must be exactly "
                f"{EXPECTED_EXECUTION_DORMANT_NOTE!r} got {dormant_note!r}",
            )
        # Ensure no extra authorization flips: check forbidden positive
        # authorizations are not true
        # The above explicit checks already kill each flip; also ensure
        # deletion fails via missing check above.
    # Ensure execution_authority block is not deleted even when Markdown
    # regenerated — deletion is already covered.
    # Inherited v1 full-projection check — deterministic hash, fail closed
    # on any drift
    # This must prove removing exact four v2-only blocks and normalizing only
    # schema_version yields exact v1
    v1_for_proj = _load_v1_for_projection(data, v1_data, v1_bytes, v1_path, repo_root, errors)
    if v1_for_proj is not None:
        _validate_inherited_projection(data, v1_for_proj, errors)
    else:
        _err(
            errors,
            "inherited v1 projection unavailable: v1 load failed (see prior v1 errors)",
        )
    # Also ensure v2-only fields are exactly validated beyond projection
    # (separate checks already above cover them)
...
def render_markdown(data: dict[str, Any]) -> str:
    ...
    out: list[str] = []
    out.append("# E3 Dynamic Resource v2 — Scientific Contract v2")
    out.append("")
    ...
    out.append("")
    # v2 successor provenance and narrow corrections rendered deterministically
    prov_raw = _get(["successor_provenance"], {})
    prov: dict[str, Any] = prov_raw if isinstance(prov_raw, dict) else {}
    occ_raw = _get(["waiting_room_occupancy_transition"], {})
    occ: dict[str, Any] = occ_raw if isinstance(occ_raw, dict) else {}
    comp_raw = _get(["comparator_rejection_precedence"], {})
    comp_dict: dict[str, Any] = comp_raw if isinstance(comp_raw, dict) else {}
    out.append("---")
    out.append("")
    out.append("## 12b. Successor provenance (v2)")
    out.append("")
    out.append(f"- successor_version: `{prov.get('successor_version', 'v2')}`")
    out.append(
        f"- superseded_candidate_sha: "
        f"`{prov.get('superseded_candidate_sha', EXPECTED_SUCCESSOR_CANDIDATE_SHA)}`"
    )
    out.append(
        f"- superseded_promoted_integration_sha: "
        f"`{prov.get('superseded_promoted_integration_sha', EXPECTED_SUCCESSOR_INTEGRATION_SHA)}`"
    )
    out.append(f"- v1_json_sha256: `{prov.get('v1_json_sha256', EXPECTED_V1_JSON_SHA256)}`")
    v1_path = prov.get(
        "v1_contract_path",
        "docs/evaluation/e3/e3_dynamic_resource_v2_contract_v1.json",
    )
    out.append(f"- v1_contract_path: `{v1_path}`")
    exact_reas = prov.get(
        "exact_reasons",
        "waiting-room occupancy-transition omission and comparator rejection-taxonomy omission",
    )
    out.append(f"- exact_reasons: `{exact_reas}`")
    out.append(
        "- reasons: waiting-room occupancy-transition omission; comparator rejection-taxonomy "
        "omission"
    )
    succ_branch = prov.get("successor_authoring_branch", EXPECTED_SUCCESSOR_AUTHORING_BRANCH)
    succ_base = prov.get("successor_prepared_base_sha", EXPECTED_SUCCESSOR_PREPARED_BASE_SHA)
    out.append(f"- successor_authoring_branch: `{succ_branch}`")
    out.append(f"- successor_prepared_base_sha: `{succ_base}`")
    out.append("")
    out.append("---")
    out.append("")
    out.append("## 12c. Waiting-room occupancy drain (v2 correction)")
    out.append("")
    out.append("- per_rsu_once_per_1000ms_outer_tick_after_all_five_task_slots: true")
    out.append("- uses_true_pre_drain_state_never_stale_view: true")
    out.append(
        "- B_pre = true pre-drain backlog_work_ms; L_pre = true pre-drain waiting-room task count"
    )
    drain_eq = occ.get("drain_work_ms", "D = min(B_pre, active_capacity_units * 1000)")
    out.append(f"- drain_work_ms: `{drain_eq}`")
    out.append("- backlog_post: `B_post = B_pre - D`")
    out.append("- if B_post <= 0: tasks_drained = L_pre, L_post = 0")
    out.append(
        "- else (B_post > 0): tasks_drained = floor(L_pre * D / max(B_pre, 1e-6)), L_post = "
        "max(L_pre "
        "- tasks_drained, 1)"
    )
    out.append(
        "- floor_term_is_tasks_removed_not_remaining_occupancy: true (floor term is tasks removed)"
    )
    out.append(
        "- dynamic_capacity_changes_work_drain_and_indirectly_released_occupancy: true; "
        "never_multiplies_waiting_room_ceiling "
        "6220"
    )
    out.append(
        "- full_work_drain_forces_zero_occupancy: true; "
        "positive_residual_work_retains_at_least_one_task: "
        "true"
    )
    out.append("- fixed_1x_must_reproduce_frozen_E2d_exactly: true")
    out.append(
        "- simulated_count_transition_does_not_make_lifecycle_available: "
        "started/compute_completed/returned/dropped "
        "remain null"
    )
    out.append("- pin_vectors:")
    pin_vecs = occ.get("pin_vectors", [])
    if isinstance(pin_vecs, list) and len(pin_vecs) == 3:
        for vec in pin_vecs:
            if isinstance(vec, dict):
                out.append(
                    f"  - B={vec.get('B_pre')} L={vec.get('L_pre')} u={vec.get('u')} -> "
                    f"D={vec.get('D')} B_post={vec.get('B_post')} "
                    f"tasks_drained={vec.get('tasks_drained')} L_post={vec.get('L_post')}"
                )
    else:
        out.append("  - B=4000 L=4 u=1 -> D=1000 B_post=3000 tasks_drained=1 L_post=3")
        out.append("  - B=500 L=4 u=3 -> D=500 B_post=0 tasks_drained=4 L_post=0 (full drain)")
        out.append("  - B=4000 L=1 u=1 -> D=1000 B_post=3000 tasks_drained=0 L_post=1")
    out.append("")
    out.append("---")
    out.append("")
    out.append("## 12d. Comparator rejection precedence (v2 correction)")
    out.append("")
    out.append(f"- applies_to: `{comp_dict.get('applies_to', ['ingress_dla', 'per_task_dla'])}`")
    out.append(
        "- preserves_frozen_E2d_taxonomy_exactly: true; "
        "p2c_approved_n0_taxonomy_remains_separate_and_unchanged: "
        "true"
    )
    out.append("- precedence:")
    out.append("  1. select comparator target;")
    out.append("  2. inactive/non-V2I yields no E3 V2I rejection class;")
    out.append(
        "  3. current radio failure OR target already full at task-slot entry -> v2i_unavailable;"
    )
    out.append("  4. otherwise strict observed-backlog gate failure -> v2i_gate_rejected;")
    out.append(
        "  5. otherwise live in-slot cap failure caused by prior admitted reservations -> "
        "v2i_cap_rejected;"
    )
    out.append("  6. otherwise admit;")
    out.append("  7. simultaneous gate and live cap failure is gate rejection;")
    out.append(
        "  8. rejected work never changes true workload, occupancy, or overlay and execution is -1."
    )
    out.append(
        "- P2C's approved n=0 taxonomy remains separate and unchanged; comparator precedence never "
        "replaces "
        "it."
    )
    cross = comp_dict.get("cross_strategy_comparability", {})
    if not isinstance(cross, dict):
        cross = {}
    out.append(
        "- cross_strategy_comparability: total rejection/rejection share are comparable but "
        "individual "
        "V2I rejection classes are mechanism diagnostics and must not be presented as identically "
        "constructed "
        "cross-strategy contrasts"
    )
    out.append("")
    out.append("---")
    out.append("")
    out.append("## 12e. Execution authority — HARD scientific execution hold (v2)")
    out.append("")
    ea_raw = _get(["execution_authority"], {})
    ea_md: dict[str, Any] = ea_raw if isinstance(ea_raw, dict) else {}
    out.append(f"- status: `{ea_md.get('status', 'E3_SCIENTIFIC_EXECUTION_NOT_AUTHORIZED')}`")
    out.append(
        "- scientific_execution_authorized: "
        f"`{ea_md.get('scientific_execution_authorized', False)}`"
    )
    out.append(f"- lane_09: `{ea_md.get('lane_09', 'BLOCKED_BY_RESEARCHER_EXECUTION_HOLD')}`")
    out.append(
        "- engineering_functionality_authorized: "
        f"`{ea_md.get('engineering_functionality_authorized', True)}`"
    )
    out.append(
        f"- minimal_construct_smoke_only: `{ea_md.get('minimal_construct_smoke_only', True)}`"
    )
    out.append(
        "- manchester_trace_comparative_execution_authorized: "
        f"`{ea_md.get('manchester_trace_comparative_execution_authorized', False)}`"
    )
    out.append(
        "- full_3600_step_cells_authorized: "
        f"`{ea_md.get('full_3600_step_cells_authorized', False)}`"
    )
    out.append(
        f"- e3a_authorized: `{ea_md.get('e3a_authorized', False)}` "
        f"e3b_authorized: `{ea_md.get('e3b_authorized', False)}` "
        f"e3c_authorized: `{ea_md.get('e3c_authorized', False)}`"
    )
    out.append(
        "- reduced_pilot_exploratory_campaign_authorized: "
        f"`{ea_md.get('reduced_pilot_exploratory_campaign_authorized', False)}`"
    )
    out.append(
        "- multi_seed_or_fleet_draw_execution_authorized: "
        f"`{ea_md.get('multi_seed_or_fleet_draw_execution_authorized', False)}`"
    )
    out.append(
        "- performance_evidence_benchmark_authorized: "
        f"`{ea_md.get('performance_evidence_benchmark_authorized', False)}`"
    )
    out.append(
        "- empirical_e3_results_authorized: "
        f"`{ea_md.get('empirical_e3_results_authorized', False)}`"
    )
    out.append(
        "- statistical_inference_authorized: "
        f"`{ea_md.get('statistical_inference_authorized', False)}`"
    )
    out.append(f"- research_workloads_launched: `{ea_md.get('research_workloads_launched', 0)}`")
    out.append(f"- evidence_state: `{ea_md.get('evidence_state', 'NOT_EXECUTED')}`")
    out.append(
        "- result_availability: "
        f"`{ea_md.get('result_availability', 'NO_E3_RESEARCH_RESULTS_AVAILABLE')}`"
    )
    out.append(
        f"- smoke_is_scientific_evidence: `{ea_md.get('smoke_is_scientific_evidence', False)}`"
    )
    out.append(
        "- software_completion_depends_on_experiment: "
        f"`{ea_md.get('software_completion_depends_on_experiment', False)}`"
    )
    out.append(
        "- hold_release_authority: "
        f"`{ea_md.get('hold_release_authority', 'future_explicit_researcher_instruction_only')}`"
    )
    out.append(
        "- planned_56_cell_design_is_dormant_predeclaration: "
        f"`{ea_md.get('planned_56_cell_design_is_dormant_predeclaration', True)}`"
    )
    dorm_note = ea_md.get(
        "dormant_predeclaration_note",
        (
            "The planned 56-cell design is retained only as a dormant "
            "predeclaration; planned cells are not execution authority and "
            "cannot run under this hold."
        ),
    )
    out.append(f"- dormant_predeclaration_note: `{dorm_note}`")
    out.append(
        "- Note: The planned 56-cell design is retained only as a dormant "
        "predeclaration; planned cells are not execution authority and cannot "
        "run under this hold."
    )
    out.append(
        "- Lane 08 may continue engineering plus minimal correctness-only "
        "smoke, but no Manchester trace comparative execution; Lanes 10-12 "
        "proceed with honest not-executed state and no placeholder results."
    )
    out.append("")
    out.append("---")
    out.append("")
    ...
    out.append("")
    out.append(
        "*End of normative contract v2 — Markdown is generated view; JSON is authoritative.*"
    )
    out.append("")
    return "\n".join(out)
...
def validate_markdown_contains(md_text: str, data: dict[str, Any]) -> list[str]:
    ...
    checks = [
        ...
        (EXPECTED_SUCCESSOR_CANDIDATE_SHA, "superseded candidate SHA in markdown"),
        (EXPECTED_SUCCESSOR_INTEGRATION_SHA, "superseded integration SHA in markdown"),
        (EXPECTED_V1_JSON_SHA256, "v1 JSON SHA256 in markdown"),
        ("waiting-room occupancy-transition omission", "waiting-room omission reason in markdown"),
        ("comparator rejection-taxonomy omission", "comparator omission reason in markdown"),
        (EXPECTED_SUCCESSOR_AUTHORING_BRANCH, "successor authoring branch in markdown"),
        (EXPECTED_SUCCESSOR_PREPARED_BASE_SHA, "successor prepared base SHA in markdown"),
        (
            "Any post-observation source change requires successor: True",
            "any_post_observation successor true in markdown",
        ),
        ("Monetary cost tested: False", "monetary cost false in markdown"),
        ('"future_leakage_forbidden": true', "future_leakage_forbidden true in canonical block"),
        (
            '"floor_removal_is_tasks_drained": true',
            "floor_removal_is_tasks_drained true in canonical block",
        ),
        (
            '"p2c_uses_v2i_unavailable_v2i_gate_rejected_v2i_cap_rejected_for_n0_only": true',
            "p2c_uses_v2i_*_for_n0_only true in canonical block",
        ),
        ("B_pre", "B_pre in markdown"),
        ("L_pre", "L_pre in markdown"),
        ("D = min(B_pre, active_capacity_units * 1000)", "D equation in markdown"),
        ("B_post = B_pre - D", "B_post equation in markdown"),
        ("floor(L_pre * D / max(B_pre, 1e-6))", "floor occupancy drain in markdown"),
        ("max(L_pre - tasks_drained, 1)", "max retain one in markdown"),
        ("v2i_unavailable", "v2i_unavailable in markdown"),
        ("v2i_gate_rejected", "v2i_gate_rejected in markdown"),
        ("v2i_cap_rejected", "v2i_cap_rejected in markdown"),
        (
            "current radio failure OR target already full at task-slot entry",
            "comparator precedence v2i_unavailable in markdown",
        ),
        (
            "strict observed-backlog gate failure -> v2i_gate_rejected",
            "comparator precedence gate in markdown",
        ),
        (
            "live in-slot cap failure caused by prior admitted reservations -> v2i_cap_rejected",
            "comparator precedence cap in markdown",
        ),
        (
            "simultaneous gate and live cap failure is gate rejection",
            "simultaneous gate precedence in markdown",
        ),
        (
            "rejected work never changes true workload, occupancy, or overlay and execution is -1",
            "rejected work never changes in markdown",
        ),
        (
            "total rejection/rejection share are comparable",
            "total rejection comparable in markdown",
        ),
        (
            "individual V2I rejection classes are mechanism diagnostics",
            "individual classes diagnostics in markdown",
        ),
        (
            "must not be presented as identically constructed cross-strategy contrasts",
            "cross-strategy contrast forbidden in markdown",
        ),
        (
            "E3_SCIENTIFIC_EXECUTION_NOT_AUTHORIZED",
            "E3 execution not authorized status in markdown",
        ),
        ("BLOCKED_BY_RESEARCHER_EXECUTION_HOLD", "lane_09 blocked by researcher hold in markdown"),
        ("future_explicit_researcher_instruction_only", "hold release authority in markdown"),
        ("NOT_EXECUTED", "evidence_state NOT_EXECUTED in markdown"),
        ("NO_E3_RESEARCH_RESULTS_AVAILABLE", "result availability no results in markdown"),
        (
            "planned cells are not execution authority and cannot run under this hold",
            "dormant planned cells not execution authority in markdown",
        ),
        (
            "retained only as a dormant predeclaration",
            "retained only as dormant predeclaration in markdown",
        ),
        ("scientific_execution_authorized", "scientific_execution_authorized in markdown"),
        (
            "engineering_functionality_authorized",
            "engineering_functionality_authorized in markdown",
        ),
        ("minimal_construct_smoke_only", "minimal_construct_smoke_only in markdown"),
        (
            "manchester_trace_comparative_execution_authorized",
            "manchester trace comparative execution authorized in markdown",
        ),
        ("full_3600_step_cells_authorized", "full_3600_step_cells_authorized in markdown"),
        ("e3a_authorized", "e3a_authorized in markdown"),
        ("e3b_authorized", "e3b_authorized in markdown"),
        ("e3c_authorized", "e3c_authorized in markdown"),
        ("research_workloads_launched", "research_workloads_launched in markdown"),
        ("evidence_state", "evidence_state in markdown"),
        ("result_availability", "result_availability in markdown"),
        ("smoke_is_scientific_evidence", "smoke_is_scientific_evidence in markdown"),
        (
            "software_completion_depends_on_experiment",
            "software_completion_depends_on_experiment in markdown",
        ),
        ("hold_release_authority", "hold_release_authority in markdown"),
        (
            "planned_56_cell_design_is_dormant_predeclaration",
            "planned_56_cell_design_is_dormant_predeclaration in markdown",
        ),
        (
            '"status": "E3_SCIENTIFIC_EXECUTION_NOT_AUTHORIZED"',
            "execution_authority status in canonical block",
        ),
        ('"lane_09": "BLOCKED_BY_RESEARCHER_EXECUTION_HOLD"', "lane_09 in canonical block"),
        (
            '"hold_release_authority": "future_explicit_researcher_instruction_only"',
            "hold_release_authority in canonical block",
        ),
        ('"research_workloads_launched": 0', "research_workloads_launched 0 in canonical block"),
        ('"evidence_state": "NOT_EXECUTED"', "evidence_state NOT_EXECUTED in canonical block"),
        (
            '"result_availability": "NO_E3_RESEARCH_RESULTS_AVAILABLE"',
            "result_availability in canonical block",
        ),
    ]
...
def main() -> int:
    ...
    parser.add_argument(
        "--contract-json",
        type=Path,
        default=Path("docs/evaluation/e3/e3_dynamic_resource_v2_contract_v2.json"),
    )
    parser.add_argument(
        "--contract-md",
        type=Path,
        default=Path("docs/evaluation/e3/e3_dynamic_resource_v2_contract_v2.md"),
    )
    parser.add_argument(
        "--v1-contract-json",
        type=Path,
        default=None,
        help=(
            "explicit v1 contract path for byte-hash and projection "
            "(default: provenance path resolved safely within repo)"
        ),
    )
    parser.add_argument(
        "--repo-root",
        type=Path,
        default=None,
        help="explicit repo root for safe v1 path resolution (default: parent of scripts/)",
    )
    parser.add_argument(
        "--check-equivalence",
        action="store_true",
    ...
    # Resolve repo root for v1 safe resolution
    repo_root = args.repo_root if args.repo_root is not None else _repo_root_default()
    v1_path_explicit = args.v1_contract_json
    result = validate_contract(data, v1_path=v1_path_explicit, repo_root=repo_root)
