...
SPEC = importlib.util.spec_from_file_location(
    "e3a_reference",
    Path(__file__).parents[2] / "docs/evaluation/e3a_csf3_2026-09-11/qualification_reference.py",
)
...
def archives(tmp_path: Path) -> tuple[Path, Path]:
    ...
    for dest in (ref, got):
        ...
        np.savez(
            dest / "per_step.npz",
            veh_action=np.array([1], np.int8),
            veh_actor_logits=np.array([0.0], np.float32),
        )
        np.savez(
            dest / "per_task.npz",
            task_met=np.array([True]),
            task_lat_ms=np.array([100.0], np.float32),
        )
    (ref / "validation.json").write_text(
        json.dumps(
            {
                "status": "passed",
                "sha256": {
                    name: REF.sha(ref / name)
                    for name in ["summary.json", "command.json", "per_step.npz", "per_task.npz"]
                },
            }
        )
    )
...
def test_new_instrumentation_does_not_hide_discrete_drift(archives: tuple[Path, Path]) -> None:
    ref, got = archives
    np.savez(
        got / "per_task.npz",
        task_met=np.array([False]),
        task_lat_ms=np.array([100.0], np.float32),
        extra=np.array([99]),
    )
...
def test_reference_tamper_is_not_a_new_baseline(archives: tuple[Path, Path]) -> None:
    ref, got = archives
...
def test_missing_inherited_field_cannot_pass(archives: tuple[Path, Path]) -> None:
    ref, got = archives
...
def test_logit_tolerance_and_new_telemetry(archives: tuple[Path, Path]) -> None:
    ref, got = archives
    np.savez(
        got / "per_step.npz",
        veh_action=np.array([1], np.int8),
        veh_actor_logits=np.array([9e-6], np.float32),
        extra=np.array([42]),
    )
...
def cpu_receipts(archives: tuple[Path, Path]) -> None:
    for index, directory in enumerate(archives):
        ...
        receipt = {
            ...
            "source_commit": "a" * 40,
            "seal_sha256": "b" * 64,
            "shared_input_hashes": {"tasks": "c" * 64},
...
def test_cpu_smoke_cannot_accept_other_valid_sampled_pair(archives: tuple[Path, Path]) -> None:
    ref, got = archives
...
def test_cpu_smoke_accepts_cpu_change_but_rejects_thread_change(
