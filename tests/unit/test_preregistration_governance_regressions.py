...
def _base_plan(**overrides: object) -> StudyPlan:  # noqa: ANN003
    base = StudyPlan(
        plan_id="gov-001",
        study_question=StudyQuestion(
            text="Governance regression: does variation change outcome under replication?",
        ...
        estimand=EstimandDefinition(
            estimand_id="est-001",
            description="Mean difference over common seeds with sufficient length.",
        ...
        cohort_rules=[
            CohortRule(rule_id="cohort-001", description="Include all valid tasks for governance.")
        ],
        exclusion_rules=[
            ExclusionRule(
                rule_id="exclude-001", description="Exclude invalid tasks for governance."
            )
        ],
        ...
        stopping_rule=StoppingRule(
            description="Fixed sample with no interim looks; observe all replicates for governance.",  # noqa: E501
            max_replicates=3,
        ),
        decision_rule=DecisionRule(
            rule_type="two_sided_test",
            alpha=0.05,
            interpretation="Reject if p <0.05; non-significance not equivalence for governance.",
...
def _frozen_plan(**overrides: object) -> StudyPlan:  # noqa: ANN003
    return freeze_plan(_base_plan(**overrides), clock=lambda: FIXED)


def _all_attachments(
    plan: StudyPlan, admitted: bool, label: ArtifactAdmission
...
def test_blocker2_pre_evidence_amendment() -> None:
    ...
    child = create_amendment(
        parent,
        changes={
            "limitations": "Pre-evidence child limitations with sufficient length for governance."
        },
        amendment_reason="Pre-evidence amendment reason sufficient for governance.",
    )
...
def test_blocker2_post_evidence_child() -> None:
    ...
    child = create_amendment(
        with_ev,
        changes={"limitations": "Post child limitations with sufficient length for governance."},
        amendment_reason="Post-evidence amendment reason sufficient for governance.",
    )
...
def test_blocker2_grandchild_retains_post() -> None:
    ...
    child = create_amendment(
        with_ev,
        changes={"limitations": "Child limitations with sufficient length for governance check."},
        amendment_reason="Child post-evidence reason sufficient for governance.",
    )
    ...
    grandchild = create_amendment(
        child_frozen,
        changes={
            "limitations": "Grandchild limitations with sufficient length for governance check."
        },
        amendment_reason="Grandchild post-evidence reason sufficient for governance.",
    )
...
def test_blocker2_three_generations() -> None:
    parent = _frozen_plan()
    with_ev = attach_evidence(
        parent, _all_attachments(parent, True, ArtifactAdmission.ADMITTED)[:1], clock=lambda: LATER
    )
    c1 = create_amendment(
        with_ev,
        changes={"limitations": "Gen1 limitations with sufficient length for governance."},
        amendment_reason="Gen1 post-evidence reason sufficient for governance.",
    )
    c1f = freeze_plan(c1, clock=lambda: LATER)
    c2 = create_amendment(
        c1f,
        changes={"limitations": "Gen2 limitations with sufficient length for governance."},
        amendment_reason="Gen2 post-evidence reason sufficient for governance.",
    )
    assert c2.revision_history[-1].is_post_evidence is True
    c2f = freeze_plan(c2, clock=lambda: LATER)
    c3 = create_amendment(
        c2f,
        changes={"limitations": "Gen3 limitations with sufficient length for governance."},
        amendment_reason="Gen3 post-evidence reason sufficient for governance.",
    )
...
def test_blocker2_pre_chain_stays_pre() -> None:
    ...
    c1 = create_amendment(
        parent,
        changes={"limitations": "C1 pre limitations with sufficient length for governance."},
        amendment_reason="C1 pre reason sufficient for governance.",
    )
    c1f = freeze_plan(c1, clock=lambda: LATER)
    c2 = create_amendment(
        c1f,
        changes={"limitations": "C2 pre limitations with sufficient length for governance."},
        amendment_reason="C2 pre reason sufficient for governance.",
    )
...
def test_blocker2_export_import_retains_post() -> None:
    ...
    child = create_amendment(
        with_ev,
        changes={"limitations": "Export child limitations with sufficient length for governance."},
        amendment_reason="Export child post reason sufficient for governance.",
    )
    ...
    grand = create_amendment(
        imported_frozen,
        changes={"limitations": "Grand export limitations with sufficient length."},
        amendment_reason="Grand export post reason sufficient for governance.",
    )
...
def test_blocker3_no_cells_derives() -> None:
    ...
    assert len(frozen.planned_run_cells) == len(plan.planned_arms) * (
        len(plan.seeds) if plan.seeds else 1
    ) * (len(plan.policies) if plan.policies else 1) * len(plan.replication_ids) * len(
...
def test_blocker4_amended_child_anchor() -> None:
    ...
    child = create_amendment(
        parent,
        changes={"limitations": "Child limitations with sufficient length for anchor test."},
        amendment_reason="Child amendment reason sufficient for anchor test.",
    )
...
def test_blocker8_seeds_all_represented() -> None:
    plan = _base_plan(
        seeds=["a", "b", "c"], policies=["p1"], planned_arms=["baseline", "variation"]
    )
...
def test_blocker8_permuting_input_same_canonical_matrix() -> None:
    ...
    m1 = {
        (c.arm_id, c.seed_id, c.policy_label, c.replication_id, c.metric_key)
        for c in build_run_matrix(plan1)
    }
    m2 = {
        (c.arm_id, c.seed_id, c.policy_label, c.replication_id, c.metric_key)
        for c in build_run_matrix(plan2)
    }
    assert m1 == m2
    ...
    plan = _base_plan(
        ...
        primary_outcomes=[
            OutcomeDefinition(
                ...
                unit="ratio",
                denominator="generated_tasks",
                description="Primary 1 for size calc.",
            ),
            OutcomeDefinition(
                outcome_id="p2",
                ...
                description="Primary 2 for size calc.",
            ),
        ],
    )
    # 2*2*2*2*2=32
...
def test_blocker9_range_cardinality_with_seeds_policies() -> None:
    ...
    plan_ok = _base_plan(
        seeds=["a", "b"],
        policies=["p1", "p2"],
        replication_generation_rule="range:100",
        replication_ids=[],
        planned_arms=["baseline", "variation"],
    )
    assert len(build_run_matrix(plan_ok)) == 800
    # Now huge: 2*2*2*2000*1=16000 >10000 should fail
    plan_huge = _base_plan(
        seeds=["a", "b"],
        policies=["p1", "p2"],
        replication_generation_rule="range:2000",
        replication_ids=[],
        planned_arms=["baseline", "variation"],
    )
    with pytest.raises(ValueError, match="exceeds limit"):
...
def test_blocker11_ready_requires_flags() -> None:
    ...
    with pytest.raises(ValueError):
        DecisionGateReport(
            status=DecisionGateStatus.READY, is_ready=False, is_blocked=False, is_unavailable=False
        )
    with pytest.raises(ValueError):
        DecisionGateReport(
            status=DecisionGateStatus.READY, is_ready=True, is_blocked=True, is_unavailable=False
        )
    with pytest.raises(ValueError):
        DecisionGateReport(
            status=DecisionGateStatus.BLOCKED, is_ready=True, is_blocked=True, is_unavailable=False
        )
    with pytest.raises(ValueError):
        DecisionGateReport(
    ...
    # Valid
    ok = DecisionGateReport(
        status=DecisionGateStatus.READY, is_ready=True, is_blocked=False, is_unavailable=False
    )
    assert ok.status == DecisionGateStatus.READY
    ok2 = DecisionGateReport(
        status=DecisionGateStatus.BLOCKED, is_ready=False, is_blocked=True, is_unavailable=False
    )
