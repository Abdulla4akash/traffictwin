...
def test_data_contract_workbench_page_renders() -> None:
    ...
    app = AppTest.from_file(f"src/traffictwin/ui/{page_script_for(UiPage.DATA_CONTRACT_WORKBENCH)}")
    ...
    assert any("Data Contract Workbench" in str(title.value) for title in app.title) or any(
        "Data Contract Workbench" in str(header.value) for header in app.header
...
def test_data_contract_workbench_candidate_vs_frozen_state() -> None:
    ...
    contract = SourceDataContract(
        ...
        fields=[FieldContract(field_name="a", required=True, logical_type=LogicalType.STRING), FieldContract(field_name="b", required=True, logical_type=LogicalType.STRING)],
    ...
    assert any("blocked" in str(metric.label).lower() for metric in app.metric) or any(
        "BLOCKED" in str(w.value) for w in app.warning
    ) or any("BLOCKED" in str(e.value) for e in app.error) or any(
        "BLOCKED" in str(w.value) for w in app.warning
    )
...
def test_data_contract_workbench_blocked_review_compatible_summaries() -> None:
    ...
    report = SchemaDriftReport(
        ...
        findings=[
            SchemaDriftFinding(field_name="a", severity=SchemaDriftSeverity.BLOCKED, code="REQUIRED_FIELD_REMOVED", message="blocked"),
            SchemaDriftFinding(field_name="b", severity=SchemaDriftSeverity.REVIEW_REQUIRED, code="OPTIONAL_FIELD_ADDED", message="review"),
            SchemaDriftFinding(field_name="c", severity=SchemaDriftSeverity.COMPATIBLE, code="TYPE_COMPATIBLE", message="compatible"),
        ],
    ...
    app.session_state["_v07_navigation_active"] = True
    app.session_state["dcw_drift_report"] = report.model_dump(mode="json")
    # Need a frozen version to show drift section? The drift section renders if drift exists regardless of frozen
    # But our UI checks drift not None, so it will render summary
    from traffictwin.data_contract.models import SourceDataContract, FieldContract, LogicalType, PublicationClass, RightsAndRetentionContract
    from traffictwin.data_contract.service import create_frozen_version
