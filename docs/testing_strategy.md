...
Verified after the `OPS-05` generalised external-source contract increment:

- 919 tests passed;
- statement coverage: 86%;
- external-source contract coverage: 95%, research-object coverage: 86%, doctor coverage: 87%,
  canonical-cache engine coverage: 84%, and registry-migration engine coverage: 92%;
- Ruff formatting/linting passed across 480 Python files;
- strict mypy passed across 473 configured source/test files;
- all 38 generated JSON references were byte-identical across two consecutive regenerations and
  all 87 generated/golden JSON documents parsed;
- dependency lock, isolated source/wheel build/install/external-contract CLI smoke, release smoke,
  152-document/1,292-local-link validation, and `git diff --check` passed. The preceding OPS-04
  desktop/mobile browser semantic audit—including Reports and Search—passed with zero findings,
  and the earlier REP-01 gate passed a real minimal Tectonic compile.
...
`scripts/ui_browser_audit.py` captures desktop Home, Replay, Diagnostics, Threshold Sensitivity,
Statistical Study, Parameter Sweep, Scenario Builder, Scenario Mutations, Comparison, Provenance,
Reports, Search, Mock Evaluation,
and a phone-sized Home view. It fails on bounded structural regressions: main-heading
count, unnamed visible controls, missing image alt attributes, duplicate DOM IDs, and horizontal
overflow. It is not a WCAG conformance audit.
...
- Ruff formatting/linting: passed across 480 Python files;
- strict mypy: passed across 473 configured source/test files;
- 38 deterministic generated JSON references: byte-identical across two consecutive runs;
- all 87 generated/golden JSON documents parsed;
- dependency lock, isolated source/wheel builds and installs, external-contract CLI smoke, release
  smoke, 152-document/1,292-local-link validation, and `git diff --check`: passed;
- the preceding OPS-04 desktop/mobile browser semantic audit, including Reports and Search, passed
  with zero findings; the earlier REP-01 gate also passed a real minimal Tectonic compile.
