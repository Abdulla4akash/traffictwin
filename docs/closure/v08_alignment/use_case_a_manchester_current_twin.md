...
| S08 | `synthetic_square_sumo` | Hand-authored TrafficTwin synthetic-square smoke scenario — not netconvert derived; Original TrafficTwin-authored synthetic scenario: one edge tracing a 100 m square perimeter with six deterministic vehicles. Not derived from Eclipse SUMO scenario files. Three admitted files pinned: `square.sumocfg` `f63508af4ac0aa9c83baaab4670cb46e6ea2f7757f523d6378f87cdf465c8a1d` (538 B), `square.net.xml` `9dba208715f894606119533ab3c6d3d95133cabe32a910eecc96fd1a47d52126` (1048 B), `square.rou.xml` `377e955571566c625c82ee43b5c2e63e55a595960a46491dfbb14d540b6ec4a1` (1048 B) | `SIMULATION OUTPUT` | `synthetic` | `src/traffictwin/integration/sumo_execution/scenario_synthetic_square/` (see `src/traffictwin/integration/sumo_execution/scenario_synthetic_square/README.md`; pins in `traffictwin.integration.sumo_execution.models` / `traffictwin.integration.sumo_execution.service._PINNED_INPUTS`) | IMPLEMENTATION-VERIFIED FACT |
...
|------|-------------------|-------------------------------|-------------|--------------------------|
| 1 | Manchester Evidence Hub (inventory, readiness, blockers) — `url_path="manchester-evidence-hub"` in `src/traffictwin/ui/navigation_v07.py` (`V07PageSpec` for `UiPage.MANCHESTER_EVIDENCE_HUB`) via `src/traffictwin/ui/app_pages/manchester_evidence_hub.py` → `src/traffictwin/ui/page_runtime.py:run_page_script` | `src/traffictwin/ui/pages/manchester_evidence_hub.py` | `/manchester-evidence-hub` | `src/traffictwin/ui/manchester_evidence_hub.py` |
| 2 | Manchester Operations — historical/latest/live-vehicle modes, source cards, map — `url_path="manchester"` in `src/traffictwin/ui/navigation_v07.py` (`MANCHESTER_PAGE_SPEC` additive) via `src/traffictwin/ui/app_pages/manchester.py` → `src/traffictwin/ui/page_runtime.py:run_manchester_page_script` → `pages/manchester_operations.render()` | `src/traffictwin/ui/pages/manchester_operations.py` | `/manchester` | `src/traffictwin/ui/manchester_operations.py` helpers; `src/traffictwin/integration/manchester/national_highways_live.py` |
| 3 | Manchester Operations — BODS live control (bus-only) — same Manchester chain `app_pages/manchester.py` (`manchester`) → `pages/manchester_operations.render()` | `src/traffictwin/ui/pages/manchester_operations.py` | `/manchester` | `src/traffictwin/integration/manchester/bods_live.py`, `src/traffictwin/integration/manchester/bods_live_control.py` |
| 4 | Manchester Operations — DfT/WebTRIS accepted catalogues and timeseries views — same Manchester chain `app_pages/manchester.py` (`manchester`) | `src/traffictwin/ui/pages/manchester_operations.py` | `/manchester` | `src/traffictwin/integration/manchester/dft_acquisition.py`, `src/traffictwin/integration/manchester/webtris_acquisition.py` |
| 5 | Scenario Builder (author bounded synthetic incident) — `url_path="scenario-builder"` in `src/traffictwin/ui/navigation_v07.py` (`V07PageSpec` for `UiPage.SCENARIO`) via `src/traffictwin/ui/app_pages/scenario_builder.py` | `src/traffictwin/ui/pages/scenario_builder.py` | `/scenario-builder` | `src/traffictwin/ui/services/scenario.py` (via `src/traffictwin/ui/services/__init__.py`), `src/traffictwin/experiments/scenario_mutation.py` |
...
- **Provenance** [IMPLEMENTATION-VERIFIED FACT]:
  ...
  revision; simulation outputs carry the hand-authored TrafficTwin synthetic-square identity — Original TrafficTwin-authored synthetic scenario: one edge tracing a 100 m square perimeter with six deterministic vehicles. Not derived from Eclipse SUMO scenario files. — with three admitted file SHA pins as in `traffictwin.integration.sumo_execution.service._PINNED_INPUTS` (`square.sumocfg` `f63508af4ac0aa9c83baaab4670cb46e6ea2f7757f523d6378f87cdf465c8a1d`, `square.net.xml` `9dba208715f894606119533ab3c6d3d95133cabe32a910eecc96fd1a47d52126`, `square.rou.xml` `377e955571566c625c82ee43b5c2e63e55a595960a46491dfbb14d540b6ec4a1`) per `src/traffictwin/integration/sumo_execution/scenario_synthetic_square/README.md` and `traffictwin.integration.sumo_execution.models`. It is not a `netconvert 1.27.1` product. Placeholders carry explicit `unavailable` provenance.
