...
**Narration (110 words):**
> Manchester is shown as a MIXED bounded demonstration, not a live deployment. The Manchester Evidence Hub at slash manchester-evidence-hub plus Manchester Operations at slash manchester. With no credentials, both render blockers: bus positions and strategic-road snapshots show not-ready-credential-missing and no retrieval is attempted; the deterministic demo replays committed receipts only. Historic DfT counts and the static ONS boundary are accepted available; TfGM signal-location layer is unavailable/design-only for the current view (full archive workspace-only/private); general live road traffic from BODS and a city-wide live twin are DESIGN-ONLY CAPABILITY and remain blocked. The view separates REAL MANCHESTER DATA from REAL EXTERNAL NON-MANCHESTER DATA — strategic-road only, never presented as Manchester city-road traffic.
