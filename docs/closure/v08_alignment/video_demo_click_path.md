...
| 3 | ~5:35 | Manchester Operations — historic baselines | `src/traffictwin/ui/pages/manchester_operations.py` (same chain) | `/manchester` | `src/traffictwin/integration/manchester/dft_acquisition.py` + `src/traffictwin/integration/manchester/boundary_reference.py` + receipts `docs/integration/evidence/manchester_dft_gate_b_probe_20260725.json` (SHA `c0a59f…`) / `docs/integration/evidence/manchester_dft_observation_acquisition_20260725.json` (SHA `f33cfe…`) | Review DfT historic + ONS boundary layers | DfT catalogue offline (committed receipts `c0a59f…` / `f33cfe…`); ONS boundary static; both labelled historical/static, never live |
