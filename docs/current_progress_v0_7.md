...
| VEC task lifecycle and hand check | `foundation_only` | Provisional native-event v1 contract plus an exact synthetic strong-link-full/weaker-link-idle two-RSU fixture; fail-closed checks reconcile node path, one execution reservation, forwarding delay/energy, 23 ms timing, deadline outcome and the single terminal task | CPU-work/service-budget conservation, producer-approved semantics, current-evaluator/native producer, replay of the hand check against native events, deterministic dispatcher, runner/admission integration and any scientific campaign |
