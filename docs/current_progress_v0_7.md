...
| VEC lifecycle, hand check, dispatch and native sidecar | `foundation_only` | Provisional lifecycle contract, exact synthetic two-RSU oracle, three-policy downstream dispatcher and a digest-bound read-only runner-sidecar verifier that replays lifecycle/dispatch artifacts and joins every V2I task/path against a successful runner receipt | CPU-work/service-budget conservation, producer-approved semantics, authorised native evaluator/adapter producer, live runner emission, admission/metrics integration, signed matched comparison, learned scheduler and any scientific campaign |
