...
| Gate D | Observation-to-SUMO mapping, calibration, accepted baseline, and observed-versus-simulated comparison | `foundation_only` | **Step 1 (network binding) is complete** under ADR-059 — approved Greater Manchester scope, osmium decode, frozen netconvert 1.27.1 builder, the built and validated full Greater Manchester network, validation, service and CLI. Remaining: approve the matching/calibration/comparison contracts; extend the synthetic mapping/review harness to accepted real evidence; build profile/baseline orchestration; register and exercise a real comparison contract | 7–14 working days after the remaining decisions |
