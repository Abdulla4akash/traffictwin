...
| VEC lifecycle, hand check, dispatch, sidecar and matched structural study | `foundation_only` | Provisional lifecycle contract, exact synthetic two-RSU oracle, three-policy downstream dispatcher, digest-bound read-only runner-sidecar validation and an all-three-policy synthetic harness that exact-matches inputs and reconciles task routes, reservations, per-RSU slot projections and caller-supplied predicted costs without selecting a winner | CPU-work/service-budget conservation, producer-approved semantics, authorised native evaluator/adapter producer, live runner emission, realised lifecycle/admission/metrics integration, signed native matched comparison, learned scheduler and any scientific campaign |
