...
| VEC lifecycle, hand check and deterministic dispatch | `foundation_only` | Provisional native-event v1 contract, exact synthetic two-RSU oracle and a common-information downstream V2I dispatcher with strongest-link/no-forwarding, least-loaded and predicted-earliest-completion policies; explicit batch reservations prevent two tasks reusing one slot | CPU-work/service-budget conservation, producer-approved semantics, current-evaluator/native producer, native replay and runner/admission integration, signed matched policy comparison, learned scheduler and any scientific campaign |
