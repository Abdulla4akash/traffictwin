...
| VEC task lifecycle | `foundation_only` | Provisional native-event v1 contract with fail-closed transition, node-continuity, fingerprint and offered/admitted task-count conservation checks over synthetic fixtures | CPU-work/service-budget conservation, producer-approved semantics, current-evaluator/native producer, hand-checked two-RSU validation, deterministic dispatcher, runner/admission integration and any scientific campaign |
