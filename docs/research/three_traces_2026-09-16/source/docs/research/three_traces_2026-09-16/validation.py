...
def validate_cell(dest,config):
 ...
 T=config['steps'];N=config['trace_dimensions']['N'];R=config['trace_dimensions']['R'];K=5;shape=(T,K,N)
 ...
 controls=dict(T=T,maxN=N,rsu_max_concurrent=6220,rsu_lb=config['arm'],fleet_seed=config['fleet_seed'],evaluator_seed=config['evaluator_seed'],enter_reset=config['trace_dimensions']['entry_channel_present'],reset_soc_on_enter=False,rsu_service_mult=config.get('service_mult',1.),rsu_backhaul_ms=0.,k8s_scale='off',fleet='uk2030',lambda_arrival=1.5,rsu_cap_mode='reject',substep_queue='sequential',veh_queue_mode='conserved',substep_queue_iterations=3,k_max=K,n_rsus=R,obs_variant='onehot17',model='C')
 ...
 with np.load(config['inputs']['trace'],allow_pickle=False) as tr:
  need(('enter' in tr.files)==config['trace_dimensions']['entry_channel_present'],'Trace entry-channel configuration mismatch')
  keep=tr['mask'][:T]&~tr['enter'][:T] if config['trace_dimensions']['entry_channel_present'] else tr['mask'][:T]
