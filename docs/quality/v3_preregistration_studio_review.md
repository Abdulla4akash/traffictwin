...
- **Whole-repo gates (worktree, `uv run`) at that rebased candidate (30214c43 era):** `ruff format --check .` pass, `ruff check .` pass (0 errors after fixing 25+24+1 E501/ANN/F841/B017 in new files), `mypy --strict src/traffictwin/preregistration` pass (Success), `mypy src/traffictwin` pass at 513 files (after page typing fixes) but `uv run mypy` (files = `src`+`tests`) reported `51 errors` at that time — this report incorrectly described them as “pre-existing, not introduced by preregistration” in the prior revision. **Correction (see §13):** at `85c784a` canonical mypy showed `37 errors in 3 files (checked 967)` — `28` in `tests/unit/test_preregistration_governance_regressions.py` + `7` in `tests/unit/test_preregistration_service.py` + `2` in `tests/integration/test_preregistration_workflow.py`, all three files created by PR #23, therefore **branch-owned** union-attr / unused-ignore, not pre-existing. Final typing cleanup makes canonical mypy green.
...
- `uv run mypy` (files = `src`+`tests`) at `85c784a` → `Found 37 errors in 3 files (checked 967 source files)` — `28` in `tests/unit/test_preregistration_governance_regressions.py` + `7` in `tests/unit/test_preregistration_service.py` + `2` in `tests/integration/test_preregistration_workflow.py`, all three files **created by PR #23** therefore **branch-owned** `union-attr` (36) + `unused-ignore` (1), **not** pre-existing as previously mischaracterised. `src/traffictwin` was already green (`513` files), but canonical gate was red. **Correction:** final typing cleanup adds explicit `assert gate_report is not None` before each `gate_report.*` dereference and removes the unused `# type: ignore` at `governance:369`; after cleanup `uv run mypy` → `Success: no issues found in 967 source files` (see chronology below).
