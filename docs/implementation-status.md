...
| VEC lifecycle and two-RSU hand check | The 6 August owner-directed foundation implements a strict native-event contract plus one synthetic reference oracle: strong-link RSU A has zero execution-slot headroom, weaker-link RSU B receives a reconciled reservation, and exact forwarding/timing/deadline/terminal-task arithmetic fails closed when altered | No CPU-work/service-budget conservation, current-evaluator adapter or native producer, producer-approved semantics, native replay of the reference case, reusable scheduler, campaign, evidence, admission or capability acceptance |
