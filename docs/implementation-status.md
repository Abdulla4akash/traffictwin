...
| VEC lifecycle, two-RSU hand check, deterministic dispatcher, native sidecar and matched structural study | The 6 August owner-directed foundation implements strict lifecycle/dispatcher contracts, a digest-bound read-only future-native verifier and an all-three-policy synthetic harness that exact-matches original inputs and reconciles per-task decisions, reservations, RSU slot projections and caller-supplied predicted costs without winner ranking | The current pinned evaluator still emits no sidecar; no authorised native producer/adapter, CPU-work/service-budget conservation, producer-approved semantics, live runner/realised-lifecycle/admission/metrics wiring, signed native matched comparison, learned scheduler, campaign, evidence or capability acceptance |
