...
| VEC lifecycle, two-RSU hand check, deterministic dispatcher and native sidecar | The 6 August owner-directed foundation implements strict lifecycle/dispatcher contracts plus a digest-bound read-only verifier that exact-binds future-native JSONL files to a successful runner receipt, replays both validators and joins V2I ingress/selection/execution/return paths | The current pinned evaluator still emits no sidecar; no authorised native producer/adapter, CPU-work/service-budget conservation, producer-approved semantics, live runner/admission/metrics wiring, signed matched comparison, learned scheduler, campaign, evidence or capability acceptance |
