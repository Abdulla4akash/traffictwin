...
| VEC lifecycle, two-RSU hand check and deterministic dispatcher | The 6 August owner-directed foundation implements a strict native-event contract, one synthetic reference oracle, and three pure downstream V2I policies over identical candidate fields; reservation-aware batches reconcile every request and prevent reuse of already reserved headroom | No CPU-work/service-budget conservation, current-evaluator adapter or native producer, producer-approved semantics, native/runner integration, signed matched comparison, learned scheduler, campaign, evidence, admission or capability acceptance |
