...
| Platform dashboard | Phase 145 implemented Inventory, Forecasts and Composer; Phase 175 added read-only Analytics Quality, Evidence Matrix, Mechanism Observatory and Decision Safety routes | No participant results, public hosting, writes or action authority |
...
Phase 175 closes the four requested backend-presentation gaps without creating an action surface.
The console reads only the immutable allowlisted analytics feed or code-registered digest-pinned
matrix, observatory and confirmed-capacity contracts. Evidence roles, exclusions, uncertainty,
execution deviations and citations remain visible; non-admitted records require explicit separate
...
Phase 193 implements the dormant `NEXT-03` aggregate-history boundary without approving its real
activation. Strict identifier-free terminal-attempt records form an owner-only canonical hash
chain with exact-retry, size, permission, duplicate, partial-write and divergence refusals. Pure
compaction binds complete 24-hour UTC buckets for both sources to explicit cadence denominators;
London projections retain offset and fold across daylight-saving changes, and future source times
become only an aggregate skew count. Private immutable day publication requires an exact
owner-approved policy fingerprint plus a caller-supplied authority validator and delay. A closed
`SAFE_ANALYSIS_SUMMARY` adapter retains descriptive, `evidence: false` standing. The release CLI
can only verify and preview. No real policy, worker wiring, long-term row, provider call, raw read,
deletion, backup schedule, public export, capability acceptance or release was created.

Phase 194 implements the pure `NEXT-04` transition boundary without acquiring or persisting a real
pair. It exact-binds accepted acquisition receipts to complete parser reports and refuses product,
scope, feed/model, evidence-class or UTC-order drift. Opaque-token set reconciliation covers first
seen, content changed, unchanged, no longer listed, validity expired and reappeared; changed fields
are closed-allowlisted, disappearance wording does not claim clearance, and literal VMS text and
measured-traffic claims remain structurally absent. The public candidate contains counts only and
remains unapproved. No real transition, writer, Changes page, public row, provider request,
capability acceptance or release was created.

Phase 195 implements the pure `NEXT-05` BODS trend boundary without persisting a real attempt.
Successful aggregates are derived only from an exact-bound verified live refresh and retain
reconciled response/membership counts, distinct-operator count, exact integer-millisecond source-
age quantiles, skew and comparable cadence; safe failures contain zero response counts. Windows
use caller-declared cadence denominators and one/no-success windows remain status-only. Vehicle,
...
## E2 Research Product — Admitted VEC Evidence (Lane 12)

Bounded VEC study re-presented as admitted product evidence: one Manchester incident hour (2024-03-15 20:00-21:00 Europe/London), four matched provisional fleet draws (seeds 1-4), evaluator seed 0, fixed 1x service with 2.5x waiting-room cap (6220/RSU), zero backhaul, frozen MAPPO actor `93c970594447efbfa76c25629307ba4bbbbacd0661f9f4423496850d899dc208` that does not observe RSU load nor choose execution RSU. Replication unit `fleet_draw`; individual tasks are accounting records, not statistical replications.

- **Base:** campaign `6e3fd0d385c20c7262e096f2d7da4995a7d9c21b`, E2 evidence `bd4570fd54ffd4e1eb21fc1d8e959190fbb103a6`, cumulative integration `4f1ef5bc82585e2a719387a71c38e188f3f43594` after 11 lanes.
- **Admission:** `ADMITTED RESEARCH` + `OWNER-AUTHORIZED PRODUCT ADMISSION` via `admit_e2_research(package).verify()`; package fingerprint `195f2e89ab4e775d1577c92a59409026fccaa2d9ebd1d973177dabd93ba83269` and receipt fingerprint `45e8c2782ff40495e472bc0e6de3ba3be1610fdb974f88b7ffd12a754d031ebc` (distinct, cryptographically bound). Not supervisor approval, not Randy confirmation. Forged receipts fail closed.
- **Artifact:** `traffictwin.resources.research.e2_resource_strategy_v1.json` loaded by `builtin_e2_research_json()` via `importlib.resources`, validated by `validate_e2_research_artifact()` then `load_admitted_builtin_e2_research()`. No fallback, no path input for the E2 preset.
- **Strategies:** `off`, `jsq`, `ingress_dla`, `dla` (one common-target per substep), `per_task_dla` (sequential per-task least-busy) via `e2_strategy_semantics()`; all `is_learned=False`, `is_deterministic=True`; Kubernetes-inspired deterministic scheduling, not actual Kubernetes deployment or cluster orchestration, not learned.
- **Comparisons:** E2b offered attainment off `0.683619229`, jsq `0.675681775`, ingress_dla `0.715773211`, dla `0.694939919` (one-draw descriptive). E2c dla_minus_ingress seeds `-0.022097034972`, `-0.020519134179`, `-0.021447383092`, `-0.020825491499` mean `-0.021222260935` CI `[-0.02233525407, -0.0201092678]` (all negative). E2d per_task_minus_ingress seeds `0.004636732564`, `0.005867285642`, `0.005071796666`, `0.005509919752` mean `0.005271433656` CI `[0.004422143925, 0.006120723387]` (all positive). E2d per_task_minus_dla mean `0.026493694591` CI `[0.026210763951, 0.026776625232]`. Bounded direction reversal only.
- **Accounting (E2d seed 1):** offered `13076234`, admitted `10594205`, rejected_total `2482029` DERIVED, forwarded `600885`, deadline_success `9475948`; offered attainment `0.724669503` headline, admitted `0.8944463506228169` diagnostic only; six unavailable `UNAVAILABLE` null with reason (gate_rejected/capacity_rejected/started/compute_completed/returned/dropped never zero).
- **Provenance/standing:** heads/manifolds/actor/trace as pinned; provenance in `E2ResearchExportBundle`; `docs/closure/e2_product_traceability.json` maps each lane to module; `docs/e2_research_product.md` is the concise user guide.
- **UI/exports:** Home `Inspect real E2 research` intent `e2` -> Resource Strategy Explorer `Load TrafficTwin E2 research` (no path) -> typed `render_e2_research` (admission, strategies, E2b/E2c/E2d, accounting, unavailable, provenance, limitations, downloads). Exports `build_e2_research_exports` JSON/CSV/Markdown deterministic (two builds byte-identical), no absolute paths/secrets/timestamps. Validator `scripts/validate_e2_research_product.py` and journey `tests/integration/test_e2_research_product_acceptance.py` enforce all.
- **Explicitly not claimed:** Kubernetes deployment/cluster orchestration, autonomous infrastructure control, learned placement/scheduler, MAPPO execution-RSU choice/load observation, Manchester-wide or population-wide performance, physical deployment/return verification, universal JSQ/per_task_dla superiority, free-flow validation, independent held-out E2d replication, task-level statistical replication, zero-backhaul realism, supervisor/research approval (see `docs/e2_research_product.md` limitations). Research workloads launched remains 0.
