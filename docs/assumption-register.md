...
| Evidence packs are the only future rules input. | `EvidencePack` embeds validation, evidence availability, metric config, metrics, and provenance; it has no narrative diagnosis fields. | confirmed | Phase 5 diagnostic rules | Keep rules decoupled from raw CSV and UI. |
