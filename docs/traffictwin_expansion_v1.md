...
| 02 | standing inflation | `SnapshotRegistration` with `status="accepted"` vs `SnapshotValidationState.ACCEPTED` — only accepted `LIVE_VEHICLE/HISTORICAL` admitted |
| 03 | BODS relabel | `BODS` family asserted as general road `vehicle_count` — BODS is bus-only, refused |
| 04 | unresolved map match | `MapMatchWorkflowResult` with `unresolved` observation silently marked accepted — workflow revalidation refuses cleared `unresolved_ids` forgery |
...
| 06 | incompatible metric | Observed `vehicle_count/vehicles_per_interval/synthetic_zone/synthetic_utc_hour` vs incompatible simulated measure/scope/time_basis — `evaluate_comparison_workflow` returns `BLOCKED` |
...
- checks expansion routes + navigation groups (honest 4/34+4),
...
Cross-epic AppTests: `tests/integration/test_expansion_journey_ui.py` (8 tests) exercises the full chain via `AppTest` without SUMO binary/network/credentials. `test_expansion_replay_truthfulness.py` (7) and `test_traffictwin_expansion_v1_acceptance.py` (8) cover the remainder (24 total).
