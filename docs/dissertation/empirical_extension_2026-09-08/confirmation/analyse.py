...
def analyse(root,seal):
 ...
 for b in seal['blocks']:
  ...
  for arm in seal['arms']:
   ...
   s=json.loads((cell/'summary.json').read_text());d[arm]=100*s['completion']
 ...
 return dict(blocks=rows,primary=results,scope='new joint-randomness study only; not pooled with prior evidence')
if __name__=='__main__':
 p=argparse.ArgumentParser();p.add_argument('--root',type=Path);p.add_argument('--output',type=Path,required=True);a=p.parse_args()
 data=analyse(a.root,json.loads((HERE/'SEALED.json').read_text())) if a.root else precision()
