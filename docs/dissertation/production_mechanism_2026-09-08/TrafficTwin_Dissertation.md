...
Master's dissertation — production-mechanism revision for author review, 8 September 2026  
Supervisor: Dr Sandra Sampaio

This revision preserves the completed gap-closure manuscript at `1b6a8555c27901ec95ad8b123c413453ecdb5620` and scientific evidence at `04f3b6a95c7bc06c80ed95b54762f12861bba183`. Newly constructed diagnostics and dated evidence-availability updates are identified separately. Submission details and assessment-specific AI-use instructions remain in the [author-review record](AUTHOR_INPUTS.md); no approval or declaration is implied.

## Abstract

Infrastructure scheduling can reverse a vehicular computing comparison even when the learned vehicle policy is frozen. TrafficTwin investigates this problem through matched evaluations of strongest-link roadside execution, common-target least-workload placement and causal per-task placement with immediate reservations. Its contribution is a precise implementation study of established scheduling mechanisms.

Across four incident fleet draws, common-target placement falls 2.122 percentage points below gate-enabled ingress, while an adaptive per-task extension exceeds it by 0.527 points. A separate four-draw morning replication excludes its inspected pilot and yields −3.232 and +3.899 points, with simultaneous intervals supporting both directions. The practical per-task gains are approximately 53 and 390 additional successes per 10,000 offered tasks.

Retrospective analysis explains destination concentration through common targets and queue clearing. It also proves that, with exact arithmetic and the two production deadline values, three simultaneous admission replacements equal a causal fixed-target scan. The earlier out-of-domain instability therefore cannot explain an exact-model production difference. Constructed float32 threshold cases qualify implementation equivalence, without establishing historical prevalence. A reduced example shows how spreading earlier work can cost a later deadline; that loss does not persist in the checked transfers to the studied RSU/substep parameters.

Matched morning outcomes locate 284,821 gains alongside 12,842 losses, including previously rejected work and admitted misses. These descriptive transitions do not identify individual-decision causal effects. Limited fleet replication, global service-work knowledge and aggregate timing restrict generalisation. The historical capacity score remains statistically inconclusive and measurement-qualified because unavailable original records leave a scoring-mask exception unquantified. Executable diagnostics and preserved source records make these distinctions inspectable.
...
The intellectual contribution is an implementation study at a defined control boundary. Established workload and reservation ideas produce opposing rankings against the same ingress comparator. Precise decision semantics and outcome accounting, rather than scheduler names or allocation balance, make that reversal interpretable.
...
The contributions are a source-audited evaluation foundation; a controlled admission/placement comparison with adaptive incident extension and separate morning replication; and a progressively tested mechanism explanation. Retrospective analysis now separates an exact-arithmetic production-domain result from floating-point behaviour and locates empirical gains alongside losses. These are implementation-study contributions using established algorithms; personal attribution remains separately assessed [[S11]](#source-s11), [[S13]](#source-s13), [[S14]](#source-s14).
...
The checkpoint's 17-dimensional observation includes task descriptors, vehicle queues, state of charge, radio summaries, compute capability, electric-vehicle status and the observed V2V target's tier. It excludes the current RSU workload vector; the actor therefore cannot directly implement infrastructure least-workload selection. The actor sees a separately sampled task descriptor and chooses one mode per active vehicle-second. That mode is reused for the independently sampled operational tasks processed during its five slots. This inherited convention limits task-conditioned policy claims; matched infrastructure comparisons preserve it.
...
The saved scalar reference separates A, simultaneous admission with common targets; B, causal admission replaying A's full target sequence; and C, causal target reselection. B is target-conditioned when queues diverge. Its 440 cases are reused without adding fleet replications.

The production-domain analysis formalises the exact map, then uses 719 additional inputs within a predeclared ceiling of 800, including recorded bounded follow-ups. Rational arithmetic tests the derived bound; coupled fixtures preserve type-specific deadlines/service ranges. Other fixtures cover zero work, capacity, extra thresholds and numerical boundaries. The unchanged helper receives explicit arrays; an instrumented prefix-arithmetic mirror exposes masks and verifies final fields against it. Actor, physical RNG generation and traffic execution are bypassed.

Discrete masks require exact agreement; the 0.001 ms offset tolerance cannot excuse changed admission. Fixture compatibility does not establish full-trajectory reachability. No raw audits, joins or full-evaluator prefixes are repeated [[S14]](#source-s14).
...
The missing intermediate masks leave a competing explanation: could three replacements themselves change admission? The retrospective exact-arithmetic analysis now bounds this possibility. With fixed targets and eligibility, each candidate depends only on earlier admissions at its destination. A causal scan therefore constructs the unique fixed point; at most n replacements settle n eligible candidates. Nonnegative work also makes the map order-reversing: adding tentative admissions can only reduce subsequent eligibility under the gate/capacity predicates.

A sharper bound is min(n, 2d−1), where d counts distinct deadline thresholds per destination. To see why, start from an upper mask containing the causal solution and consider its first extra admission. One replacement fixes the preceding prefix and rejects that candidate. A capacity rejection ends further admissions; otherwise its gate rejection permanently excludes later deadlines at or below that threshold. After two replacements the remaining unresolved suffix is again upper-bounded and has at most d−1 deadline classes. Induction gives the bound; one deadline needs only one replacement. Fixed individual ingress destinations decouple in the same way. Appendix C and the accompanying proof state the assumptions fully [[S14]](#source-s14).

Production has two distinct deadlines, 100 and 500 ms. Consequently, three replacements equal causal fixed-target admission in exact arithmetic, including capacity limits, initial backlog and arbitrary ordering of those deadlines. The earlier unstable 1/1/41/41/81 example has three thresholds and cannot establish this mechanism in the production domain. This excludes unrestricted reconciliation failure as an exact-model explanation; it does not equate fixed targets with per-task reselection.
...
Float32 qualifies that conclusion. The helper forms an exclusive prefix as inclusive cumulative sum minus the current task's work; cancellation and addition of base workload need not match a causal scan's rounding. All 324 broadly coupled fixtures had stable third masks and matched causal float32 admission, although two differed from exact rational admission near a threshold. Among 210 deliberate threshold fixtures, four differed from causal float32 admission and one alternated across replacements. The alternation survives actual array widths under imposed targets, with a locally constructible initial queue. A zero-entry fixture consistent with common-target selection instead remains stable. The alternating candidate is capacity-labelled despite spare capacity, but misses its deadline even if admitted. Errors below 0.001 ms can change a strict gate; tolerance cannot excuse a mask discrepancy. No fixture establishes full actor/RNG/traffic reachability, observed frequency or a contribution to the historical performance gap.

Target reselection can separately lose useful completions. Table 8 uses two empty RSUs, two substeps, capacity 6,220, zero transfer latency and repeated type2/type2/type1/type2 tasks with nominal production service demands. Both arms admit all eight tasks. B replays A's targets 0 then 1, whereas C chooses the current least workload after every reservation.

*Table 8. Reduced-infrastructure illustration with production task/deadline/capacity values. Every listed task is admitted in both arms; completion is diagnostic service latency in milliseconds. “Slot.task” preserves candidate order. Values shown to three decimals; tests retain full precision.*
...
B preserves an empty destination for the second substep. C spreads the first substep over both RSUs, so the later 100 ms task starts at 91.818 ms and finishes at 112.929 ms: eight successes become seven despite identical admissions. Least current workload uses no future deadline information and its backlog gate omits the candidate's own service. It therefore does not directly maximise the count meeting deadlines. Five bounded deletions/substep reductions remove this example's loss; no global minimality or optimal alternative is claimed.

Its infrastructure is reduced, however. Twelve scalar transfers to nine/ten RSUs and five substeps, retaining four/six candidates, did not retain the adverse result. The illustration isolates an order/service/deadline interaction, without claiming to explain the 12,842 recorded ingress→per-task losses or to demonstrate a loss at the full studied configuration. B is conditional on A's targets, not a proposed autonomous policy. These results sharpen the operational explanation and its limits; all original full-evaluator prefixes remain unrun [[S10]](#source-s10), [[S14]](#source-s14).
...
| Revision and retrospective diagnostics | Preserved manuscript, sources and completed audits | Personal review and acceptance pending | Exact-model proof, coupled/numerical fixtures, outcome extraction and document checks | Codex developed this retrospective analysis and drafting; no unaided student proof, execution or approval inferred |
...
Original morning validation checked matched inputs/actions, logits within 0.00001 and full/probe prefixes. Its separate workload reconstruction reported maximum accumulated discrepancy about 0.00174 ms; that dynamic audit was not rerun here. The completed raw audit separately checked recorded actions, admissions and outcomes; it is not repeated here. Logits need not be byte-identical even when actions match. These are complementary checks, not physical validation. Table 10 links each limitation to its affected claim.

*Table 10. Validity limits and their consequences for interpretation.*
...
The prior relocatable package regenerates central tables and supports explicitly supplied September raw inputs; this revision adds executable proof diagnostics and outcome extraction without requiring those arrays. Original E0/E1/E2 raw outputs remain unavailable following author-reported deletion, with no known backup. Other historical raw availability is unestablished; recovery is outside this work. The completed September inventory and audits remain valid within their recorded scope, but local presence and checksums are not off-machine preservation. An approved storage endpoint and examiner access remain unconfirmed [[S13]](#source-s13), [[S14]](#source-s14).

## 4. Conclusion

TrafficTwin establishes an implementation-dependent ranking reversal under a frozen vehicle policy. Common-target dispatch underperformed ingress, whereas causal per-task placement exceeded it in both Manchester scenarios. The contribution is a controlled evaluation and increasingly precise explanation of established scheduling mechanisms, not a new scheduler family.

For RQ1, explicit rejection improved accounting, but the archived E1 capacity-score contrast remains statistically inconclusive under its historical implementation. Interpretation as admission-consistent deadline attainment retains the unquantified off-mask exception. Surviving summaries reproduce sampling intervals; unavailable original task records prevent resolving that distinct measurement uncertainty. Equivalence is unsupported.

For RQ2, gate-enabled ingress is the stronger practical comparator. The exploratory gate contrast retains eligibility/scoring differences. Later matched incident comparisons yield common-target −2.122 and per-task +0.527 percentage points against ingress; E2d is adaptive and reuses controls. For RQ3, the separate morning replication excludes its inspected pilot and reproduces the ordering under simultaneous intervals: −3.232 and +3.899 points. Gains include previously rejected tasks and admitted misses, alongside smaller losses. Four fleet draws remain the replication units; task transitions are descriptive.

The mechanism analysis now separates three claims. Common targets bound new destinations per batch; exact gate/service/timing constraints explain queue clearing and recurring low-index destinations. Fixed-target reconciliation with two production deadline values reaches the causal mask within three replacements in exact arithmetic, excluding the old out-of-domain counterexample as its explanation. Float32 can still alter strict-threshold masks; the constructed numerical case establishes a boundary, not its historical prevalence. A reduced example also shows how spreading earlier work can cost a later deadline, but its loss did not survive the checked transfers to the studied RSU/substep parameters. Neither workload balance nor a scheduler label establishes useful completion.

For RQ4, identical 100 ms reports made the tested information treatment inactive. Live admission and immediate reservations protect the older-report model. Fixed forwarding costs reduce the per-task advantage but leave it positive at 10 ms in one draw; they do not model network congestion or arrival delay.

The strongest lessons are to specify decision and numerical semantics, verify that information treatments change observed state, and inspect gains alongside losses over offered tasks. Further claims about numerical prevalence, independent streams or physical service require additional evidence. No full-evaluator prefix was executed. Personal contribution, assessment-specific AI use and examiner access remain explicit author decisions.
...
<a id="source-s14"></a>
**S14 — Production-domain mechanism analysis.** [Model and proof](analysis/MATHEMATICS.md), [initial diagnostic protocol](analysis/PROTOCOL.md), [bounded follow-up protocol](analysis/FOLLOWUP_PROTOCOL.md), [executable diagnostics](analysis/diagnostics.py), [full results](analysis/results/DIAGNOSTICS.json), [numerical follow-up](analysis/results/NUMERICAL_FOLLOWUP.json), [adverse example and helper checks](analysis/results/ADVERSE_PRODUCTION.json), [dimension checks](analysis/results/DIMENSION_CHECK.json) and [outcome accounting](analysis/results/OUTCOME_ACCOUNTING.json). This is retrospective analysis added after the completed experiments and gap closure.

The follow-up runtime is CPython 3.11.15, JAX/JAXlib 0.4.30 and NumPy 1.26.4, CPU with x64 disabled. Historical manifests retain their own source/runtime identities. The previous compact verifier and this analysis require their declared dependencies; full reproduction additionally needs the frozen actor, traces and authorised inputs. E0/E1/E2 originals remain unavailable following author-reported deletion, with no known backup; other historical raw availability is unestablished. No recovery, upload, full simulation or human approval occurred in this revision.

**Assistance and author review.** Codex performed the prior substantive research, drafting and retrospective audits, and developed the present proof, numerical diagnostics, outcome presentation and document revision. Implementation independence is not independent student execution. The [author checklist](AUTHOR_INPUTS.md) identifies pending ownership, assessment-specific AI authority and claim/statistical review. No student reading, approval or unaided authorship is implied.
