...
\clearpage
\tableofcontents
\vfill\noindent\textbf{Word count: 9,554} (main text including tables, equations
and pseudocode; excluding captions, references, appendices and front matter);
7,860 excluding tables and pseudocode.\par
\clearpage
...
Common-target least-workload placement underperformed execution at ingress, while causal per-task placement improved it, in an adaptive incident study, a four-draw morning replication and a 32-cell confirmation on eight fresh joint fleet/evaluator-seed blocks: per-task minus ingress +4.137 percentage points, common-target minus ingress \ensuremath{-}3.504, and per-task minus causal cyclic spreading +0.631 with a Bonferroni simultaneous 95\% interval of [+0.511, +0.750]. Declared follow-ups showed per-task exceeding the evaluator's two-choice mode by +0.649, a wider advantage over spreading when server speed was halved (+0.193), the same ranking under a second actor checkpoint, and recurrence of the full ranking on weekend, evening-peak and event-night traces of the same network.
...
The findings support a bounded benefit from workload awareness beyond spreading. Two checkpoints sharing one training seed, one demand-scaled Manchester network for all five scenarios, global service-work information and aggregate timing restrict generalisation and physical interpretation. Historical capacity findings separately retain inconclusive statistics and unresolved scoring uncertainty; the later gate-enabled results do not resolve that gap.
...
Admission is itself distinct from timely completion. A finite waiting room can accept another task even when the queued service would make it late. TrafficTwin's deadline-aware gate supplements a count-only limit by inspecting backlog. An admitted task can consequently miss its deadline. I retain rejected tasks in the offered population; measuring success only among admitted work would allow selective rejection to change the population being compared.
% END SOURCE BLOCK 016

% BEGIN SOURCE BLOCK 017 paragraph
I froze the supplied multi-agent proximal policy optimisation (MAPPO) actor's upstream weights. Communication and computation jointly determine whether an offloaded task returns in time \cite{ref3}, \cite{ref6}. Matched inputs and action checks distinguish infrastructure effects from retraining and possible observation feedback. The study compares scheduling implementations supporting an existing actor \hyperref[source-s0]{[S0]}--\hyperref[source-s4]{[S4]}.
...
\caption[Experimental responsibility boundaries]{Experimental responsibility boundaries. Only admitted V2I work enters the selected execution queue; logical forwarding applies when ingress and execution differ. Resource scaling is outside the completed comparison. Reading: follow the separation between vehicle mode choice, radio ingress and infrastructure execution placement \hyperref[source-s11]{[S11]}.}\label{fig:1}
\end{figure}
...
\caption[Closest-work comparison]{Closest-work comparison. ``Work'' means remaining service demand; ``count'' means queued jobs. TrafficTwin rows describe the frozen implementation. Reading: compare decision semantics and evidence boundaries rather than equating methods by their scheduling labels.}\label{tab:1} \\
\toprule
...
I evaluate the trace-driven VEC scheduler. Section~\ref{sec:3.7} assesses the TrafficTwin product separately from the research campaigns.
...
(B) Design and implementation of a few ITS service use cases. & Three 3GPP-derived task classes, Section~\ref{sec:2.3}; five Manchester scenarios, Table~\ref{tab:2} and Section~\ref{sec:3.4}; platform workflows, Section~\ref{sec:3.7} \\
...
\caption[Morning RSU coordinates]{Morning RSU positions in local trace metres, with original zero-based indices. Reading: the nine morning locations are spatial inputs, whereas the dispatch comparisons change task destinations without moving these RSUs \hyperref[source-s8]{[S8]}.}\label{fig:2}
\end{figure}
...
\caption[Scenario identities and controls]{Scenario identity and fixed placement controls; the incident window is a simulated network-lockdown hour from \cite{ref47}. Differences between scenarios are retained and documented, not treated as a single-factor intervention. Reading: note the bundled scenario differences before attributing an effect to traffic density.}\label{tab:2} \\
\toprule
...
\caption[Offered-task lifecycle]{Offered tasks remain in the primary denominator. Admission, execution assignment and deadline outcome are separate fields. Reading: follow every offered task to an admission or terminal failure before interpreting deadline attainment.}\label{fig:3}
\end{figure}
...
\begin{xltabular}{\textwidth}{@{}YYYY@{}}
\caption[Infrastructure conditions]{Infrastructure conditions. All retain strongest-link radio ingress. ``Live'' eligibility is refreshed at each substep; the historical off mode retains step-entry coarse eligibility. Reading: follow the fixed quantities and changed decision rules for each comparison.}\label{tab:3} \\
\toprule
...
Accounting validation (E0) and capacity scoping (E1) form the validation and scoping stage. Accounting validation supplies the recorded measurement foundation. Capacity scoping compares three queue limits at fixed service over five matched draws. The exploratory single-draw comparison (E2) and ingress-with-gate control (E2b) explore placement and admission on one draw. The incident four-draw common-target replication (E2c) compares common-target with ingress on new incident seeds 1--4. After inspecting them, the incident per-task extension (E2d) adds per-task placement on those same draws and reuses their controls: an adaptive extension \hyperref[source-s0]{[S0]}--\hyperref[source-s3]{[S3]}.
...
\begin{xltabular}{\textwidth}{@{}YYYY@{}}
\caption[Method selection and alternatives]{Method selection and alternatives. Reasons refer to the implemented study, with consequences limiting interpretation. Reading: connect each selected method to its alternative, purpose and resulting evidence boundary.}\label{tab:3a} \\
\toprule
...
Archived accounting validation receipts report passing their checks. Source inspection identifies a remaining exception in the off path reused by accounting validation, capacity scoping (E1) and the exploratory single-draw comparison (E2). Coarse eligibility reaches latency scoring, while refined admission governs enqueueing. Recorded capacity rejections establish exposure, not false successes. Raw per-task arrays for accounting validation, capacity scoping and exploratory single-draw comparison were not retained; their run summaries, validation receipts and manifests are archived and reproduce the reported intervals. The task-level impact of the scoring-mask exception therefore cannot be quantified for those studies; Appendix~\ref{app:B} distinguishes this gap from later audit coverage \hyperref[source-s13]{[S13]}.
...
\caption[Four-draw morning replication]{Redrawn from archived morning primary data. Left: within-draw attainment. Right: draw differences, mean diamonds and three-contrast Bonferroni intervals. The inspected pilot is excluded; uncertainty concerns four newly declared fleet draws, conditional on the frozen actor and morning scenario. Reading: read the four new fleet draws as matched replications and the adjusted intervals as the declared contrast family \hyperref[source-s8]{[S8]}.}\label{fig:5}
\end{figure}
...
\textbf{Setting.} In incident seed 1, fresh per-task attainment was 72.47\%, versus ingress 72.00\%. The 100 ms report left attainment unchanged at 72.47\% (+0.000 points). The 500 ms and 1,000 ms reports gave 72.47\% and 72.46\%, changes of +0.002 and \ensuremath{-}0.012 points from fresh \hyperref[source-s5]{[S5]}.
% END SOURCE BLOCK 163

% BEGIN SOURCE BLOCK 164 paragraph
\textbf{Results.} Every 100 ms report equalled fresh state over 35,990 non-startup RSU/batch observations: queues had emptied, making that treatment inactive. Reports differed at 500 ms in 79.58\% of observations and at 1,000 ms in all.
...
Per-task ran at about 0.05--0.6 ms per five-substep batch against 1--6 ms for the three-replacement arms (Appendix~\ref{app:D}, Table~\ref{tab:D2}).
% END SOURCE BLOCK 169

% BEGIN SOURCE BLOCK 170 paragraph
Sparse padding traverses $N$ positions. Per-task outpaced three-pass reconciliation, and cyclic selection cost less. Section~\ref{sec:3.4} separately assesses attainment.
% END SOURCE BLOCK 170

% BEGIN SOURCE BLOCK 171 paragraph
Measured orderings differ from asymptotic work for compiler reasons set out in Appendix~\ref{app:D}.
...
\begin{xltabular}{\textwidth}{@{}YYY@{}}
\caption[Validity limits]{Validity limits and their consequences for interpretation. Reading: use each row to identify the evidence needed before extending the corresponding claim.}\label{tab:8} \\
\toprule
...
This project set out to test how infrastructure scheduling affects whether vehicle tasks meet their deadlines. This matters because useful results must arrive in time. I built and checked a frozen-actor evaluator comparison and infrastructure policies, and ran 274 full evaluator runs. In the sealed confirmation, choosing a server for each task improved deadline attainment by +4.137 percentage points over execution at the entry server and by +0.631 over cyclic spreading (Table~\ref{tab:7}). These findings concern one evaluator and one network.
...
RQ3 is limited to the completed models. The 100 ms reports equalled fresh state; older-report changes were small under live admission and reservations. A single-draw fixed-overhead calculation retained per-task's advantage over ingress through 10 ms without modelling congestion or feedback. Treatment-activation checks therefore determine what these sensitivities actually test.
% END SOURCE BLOCK 194

% BEGIN SOURCE BLOCK 195 paragraph
The exact-model bound and gate/service/drain assumptions explain recurring destinations, within Appendix~\ref{app:C}'s numerical limits. Four-draw descriptive accounting locates most gains in reduced rejection, alongside recovered admitted misses and smaller losses.
...
\bibitem{ref6} Q. Wu, W. Wang, P. Fan, Q. Fan, J. Wang and K. B. Letaief. \href{\detokenize{https://arxiv.org/abs/2311.18352}}{``URLLC-Awared Resource Allocation for Heterogeneous Vehicular Edge Computing.''} arXiv:2311.18352, 2023.
...
\bibitem{ref30} Y. Xie, Q. Wu and P. Fan. \href{\detokenize{https://arxiv.org/abs/2407.11310}}{``Digital Twin Vehicular Edge Computing Network: Task Offloading and Resource Allocation.''} arXiv:2407.11310, 2024.
...
\bibitem{ref32} E. Krijestorac, A. Memedi, T. Higuchi, S. Ucar, O. Altintas and D. Cabric. \href{\detokenize{https://arxiv.org/abs/2010.05693}}{``Hybrid Vehicular and Cloud Distributed Computing: A Case for Cooperative Perception.''} arXiv:2010.05693, 2020.
...
\bibitem{ref40} S. Huang and S. Ontañón. \href{\detokenize{https://arxiv.org/abs/2006.14171}}{``A Closer Look at Invalid Action Masking in Policy Gradient Algorithms.''} arXiv:2006.14171, 2020.
...
\bibitem{ref44} The University of Manchester. \emph{Ethics Decision Tool.} \href{\detokenize{https://www.manchester.ac.uk/research/environment/governance/ethics/approval/}}{University guidance}; \href{\detokenize{https://www.training.itservices.manchester.ac.uk/uom/ERM/ethics_decision_tool/story.html}}{decision tool}. Accessed 15 September 2026; checked by the author; outcome: ethics approval not required.
...
\begin{xltabular}{\textwidth}{@{}YYYYY@{}}
\caption[Software components and gates]{Research and product artefact inventory at source commit 1e01b75. Sizes count physical Python source lines, including comments and blank lines; hosted test outcomes are reported separately from collection. Research coverage refers to archived receipts unless explicitly labelled as a new document check \hyperref[source-s17]{[S17]}. Reading: the role and refusal gates explain what each component contributes; size alone proves neither quality nor independent authorship.}\label{tab:A1} \\
\toprule
