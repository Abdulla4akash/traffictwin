...
\clearpage
\tableofcontents
\vfill\noindent\textbf{Word count: 8,709.} References, appendices, captions and front matter
...
The research progressed from accounting and capacity to opposite ingress rankings for two least-workload implementations. Their operational difference concerns target timing, visible reservations and admission outcomes \hyperref[source-s0]{[S0]}--\hyperref[source-s4]{[S4]}.
% END SOURCE BLOCK 013

% BEGIN SOURCE BLOCK 014 figure
\begin{figure}[htbp]
\centering
...
An operator must decide whether another server can complete a vehicle task sooner than its radio-ingress server. Admission rate and communication overhead matter alongside the latency of accepted work; VEC research therefore evaluates computation and communication together \cite{ref1}, \cite{ref18}.
...
\FloatBarrier
% BEGIN SOURCE BLOCK 018 heading
...
\FloatBarrier
% BEGIN SOURCE BLOCK 031 heading
...
\FloatBarrier
% BEGIN SOURCE BLOCK 040 heading
\subsection{Scope and report structure}\label{sec:1.4}
% END SOURCE BLOCK 040

% BEGIN SOURCE BLOCK 041 paragraph
The inferential evaluation concerns the trace-driven VEC evaluator, not changes to physical Manchester traffic. The interface does not establish that the command-line research campaigns executed. TrafficTwin's Streamlit application, typed service layer, command-line workflows, scenario comparison and evidence-provenance facilities are assessed separately as software artefacts in Section~\ref{sec:3.7}.
% END SOURCE BLOCK 041

% BEGIN SOURCE BLOCK 042 paragraph
The originally planned user evaluation was not conducted and is excluded from this report's empirical claims. Consequently, usability, operator benefit and adoption are not established. The separate E3 Dynamic Resource V2 campaign also remains unexecuted; product readiness is not a research result \hyperref[source-s11]{[S11]}. Section~\ref{sec:2} specifies the method, Section~\ref{sec:3} evaluates the completed studies and artefact, and Section~\ref{sec:4} answers the objectives and prioritises evidence needed next.
% END SOURCE BLOCK 042

\FloatBarrier
% BEGIN SOURCE BLOCK 043 heading
...
\FloatBarrier
% BEGIN SOURCE BLOCK 058 heading
...
% BEGIN SOURCE BLOCK 061 equation
...
% BEGIN SOURCE BLOCK 069 equation
...
\FloatBarrier
% BEGIN SOURCE BLOCK 075 heading
...
The inherited identifiers do not define canonical scheduling algorithms: \path{jsq} selects least service workload rather than shortest task count, and \path{dla} combines that common-target selector with Equation~\eqref{eq:3} \hyperref[source-s2]{[S2]}, \hyperref[source-s4]{[S4]}. Table~\ref{tab:3} makes their execution destination and gate switch explicit.
% END SOURCE BLOCK 076

% BEGIN SOURCE BLOCK 077 table
\begingroup
\singlespacing\footnotesize\setlength{\tabcolsep}{3pt}
\begin{xltabular}{\textwidth}{@{}YYYY@{}}
...
% BEGIN SOURCE BLOCK 080 algorithm
...
% BEGIN SOURCE BLOCK 082 algorithm
\begin{algorithm}[htbp]
...
\FloatBarrier
% BEGIN SOURCE BLOCK 091 heading
...
Execution fields describe admitted assignment, not individual departures \hyperref[source-s13]{[S13]}. Original morning checks covered matched inputs/actions and full/probe prefixes, with logit tolerance 0.00001; recorded workload-balance error reached about 0.00174 ms \hyperref[source-s8]{[S8]}. The completed September audit establishes admission independently of success: active masks define offers, enqueue fields identify V2I admission, and non-V2I rejection predicates identify terminal failures. It cross-checks flags, categories, latency/deadlines, assignments and summaries.
...
\FloatBarrier
% BEGIN SOURCE BLOCK 117 heading
...
\FloatBarrier
\clearpage
% BEGIN SOURCE BLOCK 135 heading
...
% BEGIN SOURCE BLOCK 137 proposition
...
The artefact has two connected but distinct parts. The research path is a trace-driven JAX evaluator with versioned infrastructure schedulers, a sealed campaign runner and independent validation/analysis stages. The TrafficTwin product adds a Streamlit interface over typed Python services and command-line workflows for scenario construction, imports, comparison and evidence inspection. The UI exposes provenance and evidence standing rather than allowing narrative or generated recommendations to stand in for measurements \hyperref[source-s11]{[S11]}, \hyperref[source-s17]{[S17]}.
...
% BEGIN SOURCE BLOCK 152 table
\begingroup
\singlespacing\footnotesize\setlength{\tabcolsep}{3pt}
\begin{xltabular}{\textwidth}{@{}YYYYY@{}}
\caption[Software components and gates]{Research and product artefact inventory at source commit 1e01b75. Sizes count physical Python source lines, including comments and blank lines; the collected-test total is not a passing-test count. Research coverage refers to archived receipts unless explicitly labelled as a new document check \hyperref[source-s17]{[S17]}. Reading: the role and refusal gates explain what each component contributes; size alone proves neither quality nor independent authorship.}\label{tab:9} \\
\toprule
...
% BEGIN SOURCE BLOCK 173 bibliography_open
...
% BEGIN SOURCE BLOCK 199 bibliography_close
\end{thebibliography}
% END SOURCE BLOCK 199

% BEGIN SOURCE BLOCK 200 appendix_open
\appendix
...
% BEGIN SOURCE BLOCK 205 anchor
\phantomsection\label{source-s1}
% END SOURCE BLOCK 205

% BEGIN SOURCE BLOCK 206 paragraph
...
% BEGIN SOURCE BLOCK 207 anchor
\phantomsection\label{source-s2}
% END SOURCE BLOCK 207

% BEGIN SOURCE BLOCK 208 paragraph
...
% BEGIN SOURCE BLOCK 209 anchor
\phantomsection\label{source-s3}
% END SOURCE BLOCK 209

% BEGIN SOURCE BLOCK 210 paragraph
...
% BEGIN SOURCE BLOCK 240 heading
...
% BEGIN SOURCE BLOCK 242 table
\begingroup
\singlespacing\footnotesize\setlength{\tabcolsep}{3pt}
\begin{xltabular}{\textwidth}{@{}YYYY@{}}
\caption[Historical audit coverage]{Historical availability and retained gap-closure audit coverage, 8 September 2026. Historical receipts remain unchanged. Reading: compare entries under the stated denominator, controls and replication unit.}\label{tab:B1} \\
\toprule
...
% BEGIN SOURCE BLOCK 243 paragraph
...
% BEGIN SOURCE BLOCK 245 table
\begingroup
\singlespacing\footnotesize\setlength{\tabcolsep}{3pt}
\begin{xltabular}{\textwidth}{@{}YYYYYY@{}}
\caption[Matched task transitions]{Descriptive task transitions from ingress to per-task placement; four primary morning draws, pilot excluded. Reading: compare entries under the stated denominator, controls and replication unit.}\label{tab:B2} \\
\toprule
...
% BEGIN SOURCE BLOCK 248 heading
...
% BEGIN SOURCE BLOCK 253 equation
...
% BEGIN SOURCE BLOCK 257 proof_open
\begin{proof}
...
% BEGIN SOURCE BLOCK 260 proof_close
\end{proof}
% END SOURCE BLOCK 260

\FloatBarrier
% BEGIN SOURCE BLOCK 261 heading
...
% BEGIN SOURCE BLOCK 263 proof_open
\begin{proof}
...
% BEGIN SOURCE BLOCK 266 proof_close
\end{proof}
...
\FloatBarrier
% BEGIN SOURCE BLOCK 270 heading
...
\FloatBarrier
% BEGIN SOURCE BLOCK 273 heading
...
\FloatBarrier
% BEGIN SOURCE BLOCK 288 heading
...
% BEGIN SOURCE BLOCK 308 table
\begingroup
\singlespacing\footnotesize\setlength{\tabcolsep}{3pt}
\begin{xltabular}{\textwidth}{@{}YYYYYYYY@{}}
\caption[Type outcomes]{Type outcomes. I = ingress, P = per-task; offered and category counts are tasks, attainment is percent. Gate rejection and admitted misses are disjoint. Reading: compare entries under the stated denominator, controls and replication unit.}\label{tab:D1} \\
\toprule
...
% BEGIN SOURCE BLOCK 310 heading
...
\singlespacing\footnotesize\setlength{\tabcolsep}{3pt}
\begin{xltabular}{\textwidth}{@{}YYYY@{}}
\caption[Eight paired effects]{All eight paired effects, percentage points. These blocks, not tasks or RSUs, are the new replications. Reading: compare entries under the stated denominator, controls and replication unit.}\label{tab:E2} \\
\toprule
