...
### Changes and visual fixes

Tables 7a/7b are generated from the two pinned analysis objects to three decimals, with the all-ten and all-fifteen families kept separate from the original confirmation. Reference 47 is registered once from the supplied primary PDF; no placeholder DOI is promoted. All designated moves remain verbatim and the two mutation probes reject a changed outcome or reinserted deleted sentence. Layout-only converter changes keep the new tables intact, widen Table 9's evidence column, break long linked filenames and fit Figure 7 on its landscape page. No validator threshold or scientific equation was changed.

The preparation receipt is retained as a historical pre-application record; the final acceptance receipt supersedes its pending status. The owner amendments and copyright fallback are recorded in AUTHOR_ACTIONS and the operation ledger. Existing earlier receipts below retain their original artifact identities and decisions.
