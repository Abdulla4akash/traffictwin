...
def main():
    ...
    ap.add_argument("--per-task-out", default="",
                    help="npz path: per-task log [T,KMAX,N] (type, latency, "
                         "met, active) — large; use for selected runs only")
    ...
    def step(carry, xs):
        ...
        actions = jnp.where(act, jnp.argmax(apply(obs), axis=1).astype(jnp.int32), 0)
        ...
        if PERTASK:
            ...
            if SEQQ:
                ys.update(task_outcome=sub_ys[4])
        if K8S == "reactive":
    ...
    if PERSTEP or PERTASK:
        ...
        if PERSTEP:
            ...
            step_keys = ["arrivals", "done", "lat_sum", "active", "n_local",
                         "n_v2i", "n_v2v", "veh_action", "veh_k", "veh_done",
                         "veh_queue_ms", "veh_best_rsu", "veh_best_v2v",
                         "rsu_busy_ms", "rsu_load"]
            if K8S == "reactive":
