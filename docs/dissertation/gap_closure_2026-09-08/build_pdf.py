...
def inline(children):
    ...
    for t in children or []:
        if t.type == 'text':
            ...
            value = re.sub(r'\b([NLWQ])_(met|offered|admitted|failed|raw|initial|final|post|next)\b', r'\1<sub>\2</sub>', value)
            out.append(value)
    ...
    while i < len(tokens):
        ...
        if tok.type == 'table_open':
            ...
            proportions = {
                ...
                7: [.14,.20,.26,.20,.20], 8: [.26,.29,.45],
                9: [.20,.31,.25,.24], 10: [.11,.18,.18,.18,.18,.17],
            }[table_number]
