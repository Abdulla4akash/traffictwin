...
\clearpage
\tableofcontents
\vfill\noindent\textbf{Word count: 9,605} (main text including tables, equations
and pseudocode; excluding captions, references, appendices and front matter);
8,174 excluding tables and pseudocode.\par
\clearpage
{\small\singlespacing
\listoffigures
\listoftables
...
Mobility makes this harder: radio conditions and the vehicles around an RSU can change while previously admitted work still occupies its server \cite{ref5}. A good connection at offloading does not establish spare computing service. Research on heterogeneous vehicular edge computing accordingly considers communication and resource allocation together with stringent latency requirements \cite{ref6}. This motivates examining infrastructure decisions under fixed vehicle-policy weights.
...
\textbf{C1.} Identifying the mismatch between task scoring and queue admission in the inherited evaluator, repaired by Putra on 5 August 2026 and qualified in E0 (Section~\ref{sec:3.1}).
...
\textbf{C5.} Designing and executing the initial comparison of six infrastructure policies over three replication stages and more than eighty full evaluator runs, with offered-task accounting and prespecified simultaneous intervals (Sections 2.6 and 3.2--3.4).
...
I evaluate the trace-driven VEC scheduler. Section~\ref{sec:3.7} assesses the TrafficTwin product separately; its existence does not establish execution of the research campaigns or effects on physical Manchester traffic.
...
One archived MAPPO checkpoint and one device-tier preset are fixed in the initial studies; Section~\ref{sec:3.4} adds a second checkpoint. Training was not reproduced (Appendix~\ref{app:F}).
...
\begin{xltabular}{\textwidth}{@{}YYYY@{}}
\caption[Infrastructure conditions]{Infrastructure conditions. All retain strongest-link radio ingress. ``Live'' eligibility is refreshed at each substep; the historical off mode retains step-entry coarse eligibility. \path{p2c} was not evaluated; \path{dla_p2c} was added in the declared follow-ups. The other listed modes were evaluated. Reading: follow the fixed quantities and changed decision rules for each comparison.}\label{tab:3} \\
\toprule
...
Execution fields record admitted assignment, not departures \hyperref[source-s13]{[S13]}. Morning checks covered matched inputs/actions and full/probe prefixes, with logit tolerance 0.00001 and workload-balance error up to about 0.00174 ms \hyperref[source-s8]{[S8]}.
...
Scheduler-only timing follows the protocol in Appendix~\ref{app:D} over sixteen configurations.
...
E1's archived capacity contrast is statistically inconclusive under the historical implementation and retains the scoring-mask qualification; Appendix~\ref{app:B} gives the interval and the admitted-latency summaries.
...
\textbf{Setting.} The inspected morning pilot offered 1,744,116 tasks and produced 91.075\% ingress versus 94.028\% per-task attainment: +2.953 points or 51,503 successes. It motivated replication but remains exploratory \hyperref[source-s7]{[S7]}.
...
\textbf{Setting.} In incident seed 1, fresh per-task attainment was 72.46695\%, versus ingress 72.00328\%. Report ages 100, 500 and 1,000 ms yielded 72.46695\%, 72.46856\% and 72.45510\%; single-draw changes do not establish equivalence or population robustness \hyperref[source-s5]{[S5]}.
% END SOURCE BLOCK 163

% BEGIN SOURCE BLOCK 164 paragraph
\textbf{Results.} Every 100 ms report equalled fresh state over 35,990 non-startup RSU/batch observations: queues had emptied, making that treatment inactive. Reports differed at 500 ms in 79.58\% of observations and at 1,000 ms in all.  Equal outcomes do not establish harmless communication delay.
...
\appendixsection{app:H}{Artefact access and verification}
\onehalfspacing
The repository is \url{https://github.com/Abdulla4akash/traffictwin}. The manuscript branch is \path{docs/dissertation-examiner-revision-2026-09-11}, its source base is \texttt{1e01b75}, and the manuscript package is \path{docs/dissertation/examiner_revision_2026-09-11/}. The LaTeX source and its build inputs form this delivery; the tracked dissertation PDF is the preserved earlier build.

\textbf{AUTHOR\_ACTION: access mechanism.} The owner must choose and arrange examiner read access or an archive deposit. Neither mechanism nor an off-machine backup is certified here. Submission gate 3 remains open until the owner completes access and preservation verification.
