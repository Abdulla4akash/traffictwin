...
The closest comparison is parallel-server dispatch: what information estimates waiting time, and when an assignment changes it. RL supplies the upstream policy. This synthesis is retrospective: papers were inspected during manuscript revision; that reading is not presented as motivating the historical experiments.
...
The research contributes a measurement-qualified implementation comparison, separate replication and confirmation, and an exact-model explanation with numerical and information limits. Retrospective task-type and cost analyses connect attainment to outcomes and engineering trade-offs; E0/E1 provide preliminary measurement and scoping evidence. Section 3.7 assesses implementation and documented contributions [[S12]](#source-s12)–[[S16]](#source-s16).
...
The morning protocol declared four new seeds and all three arms before outcomes. Table 5 reproduces common-target < ingress < per-task in every draw. Mean attainment is 85.655%, 88.887% and 92.785%, respectively; its lower ingress/per-task means than the pilot underline the need for replication [[S8]](#source-s8).
...
All 32 full morning cells and eight block-control receipts passed after bounded qualification, with no failed attempts, retries or exclusions. Table 7 reports the declared contrasts; Appendix E retains every block [[S16]](#source-s16).
