...
## FINAL current-authority — HARD E3 scientific-execution hold (2026-08-13, Lane 01 v2 successor `worker/e3-lane-01-contract-v3` base `bbd955ed181d2a82183158ded8ad86116809bf27`) — supersedes all prior next-action

**This FINAL current-authority section explicitly supersedes the older v1 “The next thing to do” instructions above (which are historical) and any earlier draft-v2 FINAL append; append-only means base bytes are not rewritten and this EOF section is the operative authority. No earlier next-action remains active.**

- v1 is historical: `e3_dynamic_resource_v2_contract_v1` (base `80e8ae55dfbcc0aa271ed7ed1d67aeae8f384761`) is frozen historical contract; its next-action text is stale and no longer active.
- v2 is current draft, not yet approved: `e3_dynamic_resource_v2_contract_v2` (successor v2, branch `worker/e3-lane-01-contract-v3`, prepared base `bbd955ed181d2a82183158ded8ad86116809bf27`) binds v1 JSON SHA-256 `0d8c545703405c59dce5b1d1c7d38a63b31341c23364de43c6530862931fa160`, exact reasons `waiting-room occupancy-transition omission and comparator rejection-taxonomy omission` (ordered list `["waiting-room occupancy-transition omission", "comparator rejection-taxonomy omission"]`), `applies_to` exactly `["ingress_dla", "per_task_dla"]`, JSON `e3_dynamic_resource_v2_contract_v2.json` is single normative authority with deterministic Markdown `e3_dynamic_resource_v2_contract_v2.md` via `render_markdown`.
- v2 remains unapproved until controller gates, candidate commit/push, metadata equality, fresh Opus, then promotion: gates validate JSON/Markdown parity, validator, tests, byte hashes, projection, v1 byte identity, successor provenance, reasons/applies_to exactness, and RESEARCH_NEXT FINAL authority, producing a candidate commit/push, verifying metadata equality, and requiring fresh Opus independent review returning `APPROVE` for exact JSON/Markdown/validator/tests at base `bbd955ed181d2a82183158ded8ad86116809bf27` before promotion; commit/push creates the candidate and is not “only after approval” — it precedes metadata equality and Opus review, and promotion follows approval.
- Lane 07 successor is now approved/promoted: candidate `b4c8ebeadb43f4246dcf49a795ecc53222444f52`, merge `282c7f9c27be819f560644dfbc49022621e5abf0`.
- Lane 08 is blocked only pending v2 promotion, then engineering/minimal correctness smoke may proceed: Lane 08 may continue engineering functionality and minimal construct smoke that is correctness-only, but `manchester_trace_comparative_execution_authorized = false` and `full_3600_step_cells_authorized = false` prohibit any Manchester trace comparative execution, any full 3600-step cell, and any E3a/E3b/E3c execution until v2 is promoted; after promotion, engineering and minimal smoke are authorized while scientific execution remains subject to hold release.
- HARD E3 scientific-execution hold — researcher-imposed, supersedes automatic progression: execution_authority `status = E3_SCIENTIFIC_EXECUTION_NOT_AUTHORIZED`, `scientific_execution_authorized = false`, `lane_09 = BLOCKED_BY_RESEARCHER_EXECUTION_HOLD`, `engineering_functionality_authorized = true`, `minimal_construct_smoke_only = true`, `manchester_trace_comparative_execution_authorized = false`, `full_3600_step_cells_authorized = false`, `e3a_authorized = false`, `e3b_authorized = false`, `e3c_authorized = false`, `reduced_pilot_exploratory_campaign_authorized = false`, `multi_seed_or_fleet_draw_execution_authorized = false`, `performance_evidence_benchmark_authorized = false`, `empirical_e3_results_authorized = false`, `statistical_inference_authorized = false`, `research_workloads_launched = 0` (`"research_workloads_launched": 0`), `evidence_state = NOT_EXECUTED` (`"evidence_state": "NOT_EXECUTED"`), `result_availability = NO_E3_RESEARCH_RESULTS_AVAILABLE` (`"result_availability": "NO_E3_RESEARCH_RESULTS_AVAILABLE"`), `smoke_is_scientific_evidence = false`, `software_completion_depends_on_experiment = false`, `hold_release_authority = future_explicit_researcher_instruction_only`; planned 56-cell design retained only as dormant predeclaration; planned cells are not execution authority and cannot run under this hold.
- Lanes 10–12 continue without placeholders/results and software completion does not depend on experiment: Lanes 10–12 and downstream consumers proceed with `evidence_state = NOT_EXECUTED` and `result_availability = NO_E3_RESEARCH_RESULTS_AVAILABLE`, recording honest not-executed state with no placeholder results, no empirical E3 results, no statistical inference, and no performance evidence benchmark; `software_completion_depends_on_experiment = false` and `smoke_is_scientific_evidence = false` remain enforced.
- Exact successor provenance for enforcement: `successor_authoring_branch = worker/e3-lane-01-contract-v3`, `successor_prepared_base_sha = bbd955ed181d2a82183158ded8ad86116809bf27`, `v1_contract_path = docs/evaluation/e3/e3_dynamic_resource_v2_contract_v1.json`, `v1_json_sha256 = 0d8c545703405c59dce5b1d1c7d38a63b31341c23364de43c6530862931fa160`, `reasons = ["waiting-room occupancy-transition omission", "comparator rejection-taxonomy omission"]`, `exact_reasons = "waiting-room occupancy-transition omission and comparator rejection-taxonomy omission"`, `applies_to = ["ingress_dla", "per_task_dla"]`, `execution_authority.status = E3_SCIENTIFIC_EXECUTION_NOT_AUTHORIZED`, `lane_09 = BLOCKED_BY_RESEARCHER_EXECUTION_HOLD`, `research_workloads_launched = 0`, `evidence_state = NOT_EXECUTED`, `result_availability = NO_E3_RESEARCH_RESULTS_AVAILABLE`, `hold_release_authority = future_explicit_researcher_instruction_only` — any drift fails closed; only future explicit researcher instruction lifts the hold.

This FINAL current-authority section is the current authority; dated audits, older next-action blocks, and any earlier draft-v2 FINAL are snapshots and do not override it unless a new direct researcher instruction supersedes this file and this file is updated in the same reviewed change.
