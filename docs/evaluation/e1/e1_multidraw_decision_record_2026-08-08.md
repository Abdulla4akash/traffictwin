...
**Manifest SHA-256:**
`35531f397bc3ba5c93e2d49ac60b8d0f7bc016121f253bf2cc1496170ec2c66c`

**Decision:** the seed-0 scientific-design gate is closed. A later direct instruction on the same
date inserted a mandatory Colab GPU backend gate, so no campaign smoke or full cell is currently
authorised. Only the bounded backend-validation smoke is allowed.
...
comparison exists. G4 is not selected; the campaign backend remains pending and no full cell is
authorised.
...
The campaign backend is pending. The established local macOS arm64 CPU runtime is the validated
reference candidate, not the currently selected execution base. CSF3 is unavailable from this
machine because the `csf3` hostname was not resolvable on 8 August 2026. A Google Colab NVIDIA GPU
is the alternative candidate and must first pass the bounded backend manifest.
...
The earlier macOS plan used concurrency one because the runtime exposes one TFRT CPU device using
the ten-core host. Final concurrency, campaign runtime and whether seed 0 is reused or rerun remain
unset until the backend record is reviewed.
...
The backend-smoke manifest, upload procedure and campaign hold must be committed and pushed before
the Colab smoke. After its downloaded evidence is validated, the selected backend, final matrix,
seed-0 handling, concurrency and runtime estimate must be recorded in the campaign manifest and
authority pointer before any full run. Failure of any gate revokes authority for subsequent cells.
E2 and every other extension remain prohibited pending a new reviewed direction.
