...
from p2c import p2c_sequential
from e2_path_instrumentation import summarise_v2i_paths
from e2d_per_task_placement import per_task_sequential_least_busy
from rsu_state_delay import validate_delay_ms, workload_report
# Load this checkout's pure environment without the optional training wrapper
# imported by env/__init__.py. No external ~/scratch symlink is required.
REPO = str(EXTENSION_DIR / "vendor")
sys.path.insert(0, REPO)
import vec_jax as V

PARK = 1.0e7
P2C_TASK_KEYS = (
    "task_p2c_feasible_count", "task_p2c_sampled_pair",
    "task_p2c_feasibility_workload_checks", "task_p2c_ranking_workload_inspections",
    "task_p2c_unique_workload_values_observed", "task_p2c_sequential_ordinal",
    "task_p2c_placement_busy_ms",
)
...
def main():
    ...
    ap.add_argument("--rsu-lb", default="off",
                    choices=["off", "jsq", "p2c", "dla", "dla_p2c",
                             "ingress_dla", "per_task_dla", "causal_round_robin", "p2c_dla"],
                    help="infrastructure execution/admission mode: radio "
                         ...
                         "ingress_dla = strongest-link ingress execution + "
                         "the same deadline-aware admission gate (no load "
                         "balancing or forwarding), "
                         "per_task_dla = per-task sequential least-busy "
                         "placement over remaining RSU service workload + "
                         "the same deadline-aware admission gate, "
    ...
    ap.add_argument("--rsu-state-delay-ms", type=int, default=None,
                    help="optional placement-workload report age, 0..1000 ms; "
                         "requires per_task_dla, sequential/reject and fixed "
                         "service. Admission uses live queues. Explicit 0 "
                         "adds audit outputs while retaining fresh placement.")
    ap.add_argument("--substep-queue", default="snapshot",
                    ...
                    help="within-substep RSU queue accounting (2026-08-04). "
                         "snapshot = legacy: every task in an ordering substep is "
    ...
    STATE_STUDY = args.rsu_state_delay_ms is not None
    STATE_DELAY = args.rsu_state_delay_ms or 0
    if STATE_STUDY:
        try:
            validate_delay_ms(args.rsu_state_delay_ms)
        except ValueError as error:
            ap.error(str(error))
        if (args.rsu_lb != "per_task_dla"
                or args.substep_queue != "sequential"
                or args.rsu_cap_mode != "reject"
                or args.k8s_scale != "off"):
            ap.error("--rsu-state-delay-ms requires --rsu-lb per_task_dla, "
                     "--substep-queue sequential, --rsu-cap-mode reject "
                     "and --k8s-scale off")
    ...
    if STATE_STUDY:
        if "times" not in tr.files or T < 1:
            ap.error("state-delay study requires a nonempty trace with timestamps")
        times = np.asarray(tr["times"][:T])
        if (times.shape != (T,) or not np.all(np.isfinite(times))
                or not np.allclose(np.diff(times), 1.0, rtol=0.0, atol=1e-6)):
            ap.error("state-delay study requires finite one-second trace timestamps")
    ...
    if args.rsu_lb == "per_task_dla" and not (SEQQ and CAP_REJECT):
        raise SystemExit("--rsu-lb per_task_dla requires --substep-queue "
                         "sequential and --rsu-cap-mode reject")
    ROUND_ROBIN = args.rsu_lb == "causal_round_robin"
    P2C = args.rsu_lb == "p2c_dla"
    if P2C and not (SEQQ and CAP_REJECT and args.veh_queue == "conserved"
                    and K8S == "off" and not STATE_STUDY
                    and args.fleet_seed >= 0
                    and float(args.rsu_service_mult or "1") == 1.0
                    and V._RSU_MULT == 1.0):
        raise SystemExit("p2c_dla E3a requires sequential/reject/conserved, fixed 1x, fresh state and explicit fleet seed")
    ...
    def per_task_offsets(sub_keys, actions, slot_types, radio_viable, rb, rl,
                         active, svc_mult_by_rsu, placement_busy_ms=None):
        """Causal per-task placement and admission for ``per_task_dla``.

        The random-key derivation and RSU work draw exactly mirror
        ``seq_offsets``.  No new key or split enters the evaluator PRNG chain.
        """
        k5 = jax.vmap(lambda k: jax.random.split(k, 5))(sub_keys)
        raw_cms = jax.vmap(V.compute_time_ms, in_axes=(0, 0, None))(
            k5[:, 1], slot_types, jnp.int32(V.RSU_TIER_IDX))
        multipliers = (jnp.ones(R, jnp.float32) if svc_mult_by_rsu is None
                       else svc_mult_by_rsu)
        service_work_by_rsu = raw_cms[:, None] / multipliers[None, :]
        attempts = active & (actions == 1)
        result = per_task_sequential_least_busy(
            attempts=attempts,
            ingress_radio_viable=radio_viable,
            deadlines_ms=V.TASK_DEADLINE_MS[slot_types],
            service_work_ms_by_rsu=service_work_by_rsu,
            base_busy_ms=rb,
            base_load=rl,
            capacity=V.RSU_MAX_CONCURRENT,
            reconciliation_iterations=args.substep_queue_iters,
            placement_busy_ms=placement_busy_ms,
        )
        w_off = jnp.sum(jnp.where(
            attempts, result.selected_service_work_ms, 0.0))
        w_adm = jnp.sum(jnp.where(
            result.admitted, result.selected_service_work_ms, 0.0))
        return (
            result.queue_offset_ms,
            result.admitted,
            jnp.sum(result.gate_rejected.astype(jnp.float32)),
            jnp.sum(result.cap_rejected.astype(jnp.float32)),
            result.gate_rejected,
            w_off,
            w_adm,
            result.selected_rsu,
            result.coarse_candidate,
            result.placement_busy_before_ms,
        )

    def round_robin_offsets(sub_keys, actions, slot_types, radio_viable, rb, rl,
                         active, svc_mult_by_rsu, pointer):
        """Causal round-robin version 1; original service draws and admission fields.
        ...
        service_work_by_rsu = raw_cms[:, None] / multipliers[None, :]
        attempts = active & (actions == 1)
        result, next_pointer = causal_round_robin(
            attempts=attempts,
            ingress_radio_viable=radio_viable,
            deadlines_ms=V.TASK_DEADLINE_MS[slot_types],
            service_work_ms_by_rsu=service_work_by_rsu,
    ...
    def p2c_offsets(sub_keys, actions, slot_types, radio_viable, rb, rl,
                    active, outer_tick, task_slot):
        """Original RSU service draw; P2C consumes no exogenous random key."""
        k5 = jax.vmap(lambda k: jax.random.split(k, 5))(sub_keys)
        raw_cms = jax.vmap(V.compute_time_ms, in_axes=(0, 0, None))(
            k5[:, 1], slot_types, jnp.int32(V.RSU_TIER_IDX))
        service_work_by_rsu = raw_cms[:, None] / jnp.ones((1, R), jnp.float32)
        attempts = active & (actions == 1)
        result = p2c_sequential(
            attempts=attempts, ingress_radio_viable=radio_viable,
            deadlines_ms=V.TASK_DEADLINE_MS[slot_types],
            service_work_ms_by_rsu=service_work_by_rsu,
            base_busy_ms=rb, base_load=rl, capacity=V.RSU_MAX_CONCURRENT,
            reconciliation_iterations=args.substep_queue_iters,
            evaluator_seed=args.seed, fleet_seed=args.fleet_seed,
            outer_tick=outer_tick, task_slot=task_slot)
        return (result.queue_offset_ms, result.admitted,
                jnp.sum(result.gate_rejected.astype(jnp.float32)),
                jnp.sum(result.cap_rejected.astype(jnp.float32)),
                result.gate_rejected,
                jnp.sum(jnp.where(attempts, result.selected_service_work_ms, 0.0)),
                jnp.sum(jnp.where(result.admitted, result.selected_service_work_ms, 0.0)),
                jnp.maximum(result.selected_rsu, 0), result.coarse_candidate,
                result)
    ...
    def step(carry, xs):
        ...
        if ROUND_ROBIN:
            ...
            carry = carry[:-1]
        if STATE_STUDY:
            previous_post_admission_busy, step_index = carry[-1]
            carry = carry[:-1]
        if K8S == "reactive":
        ...
        if STATE_STUDY:
            report = workload_report(
                previous_post_admission_busy_ms=previous_post_admission_busy,
                live_busy_ms=rsu_busy, step_index=step_index,
        ...
        def sub_step(c, sub_idx):
            ...
            if args.rsu_lb != "off":
                # Infrastructure placement/admission modes.  Existing
                # load-balancing modes decouple execution from radio ingress;
                # ingress_dla deliberately keeps execution at that ingress and
                # enables only the same deadline-aware gate used by dla.
                if args.rsu_lb == "ingress_dla":
                    exec_idx = best_rsu_idx
                elif args.rsu_lb in ("jsq", "dla"):
                ...
                elif args.rsu_lb == "per_task_dla":
                    (rsu_extra, rsu_ok, n_grej, n_crej, gate_fail_m,
                     w_off_v2i, w_adm_v2i, exec_idx,
                     rsu_ok_pre, placement_busy_before_ms) = per_task_offsets(
                        sub_keys, actions, slot_types, v2i_q > 0.0, rb, rl,
                        active, m_cur if K8S_ON else None,
                        # All ordering substeps share one timestamp/report.
                        # rb-rsu_busy contains only this batch's acknowledged
                        # admissions; rejected work never enters the overlay.
                        report.busy_ms + (rb - rsu_busy)
                        if STATE_DELAY > 0 else None)
                elif P2C:
                    (rsu_extra, rsu_ok, n_grej, n_crej, gate_fail_m,
                     w_off_v2i, w_adm_v2i, exec_idx, rsu_ok_pre,
                     p2c_result) = p2c_offsets(
                        sub_keys, actions, slot_types, v2i_q > 0.0, rb, rl,
                        active, p2c_tick, sub_idx)
                elif ROUND_ROBIN:
                    (rsu_extra, rsu_ok, n_grej, n_crej, gate_fail_m,
                     w_off_v2i, w_adm_v2i, exec_idx, rsu_ok_pre,
                ...
                if args.rsu_lb not in ("per_task_dla", "causal_round_robin", "p2c_dla"):
                    rsu_ok = (v2i_q > 0.0) & (
                        rl[exec_idx] < V.RSU_MAX_CONCURRENT)
                    rsu_ok_pre = rsu_ok
                if SEQQ and args.rsu_lb not in ("per_task_dla", "causal_round_robin", "p2c_dla"):
                    ...
                    (rsu_extra, rsu_ok, n_grej, n_crej, gate_fail_m,
                     w_off_v2i, w_adm_v2i) = seq_offsets(
                        ...
                        args.rsu_lb in ("dla", "dla_p2c", "ingress_dla"))
                elif args.rsu_lb in ("dla", "dla_p2c", "ingress_dla"):
            ...
            if CAP_REJECT:
                ...
                rl2 = rl + rsu_inc_n
                if P2C:
                    # Preserve the causal reservations across task substeps.
                    # This also avoids repricing the float32 running sum by
                    # regrouping it into a separate scatter reduction.
                    rb2 = p2c_result.effective_busy_ms
                    rl2 = p2c_result.effective_load
            else:
            ...
            if PERTASK:
                ...
                if P2C:
                    task_selected_execution_rsu = p2c_result.selected_rsu.astype(jnp.int16)
                ...
                if STATE_STUDY:
                    ys_sub += (jnp.where(v2i_attempt, placement_busy_before_ms,
                                         jnp.float32(-1.0)),)
                if P2C:
                    ys_sub += (p2c_result.feasible_count.astype(jnp.int16),
                               p2c_result.sampled_pair.astype(jnp.int16),
                               p2c_result.feasibility_workload_checks.astype(jnp.int16),
                               p2c_result.ranking_workload_inspections.astype(jnp.int8),
                               p2c_result.unique_workload_values_observed.astype(jnp.int16),
                               p2c_result.sequential_task_ordinal,
                               p2c_result.placement_busy_before_ms)
            else:
                ys_sub = None
            if ROUND_ROBIN:
                c2 += (pointer,)
        ...
        if PERSTEP and STATE_STUDY:
            ys.update(
                state_observed_at_ms=report.observed_at_ms,
                state_captured_at_ms=report.captured_at_ms,
                state_actual_age_ms=report.actual_age_ms,
                state_warmup=report.warmup,
                rsu_report_busy_ms=report.busy_ms,
                rsu_start_busy_ms=rsu_busy,
                rsu_pre_drain_busy_ms=rb,
            )
        if PERTASK:
            ...
            if STATE_STUDY:
                ys.update(task_placement_busy_ms=sub_ys[11])
            if P2C:
                for index, name in enumerate(P2C_TASK_KEYS, start=11):
                    ys[name] = sub_ys[index]
        if K8S == "reactive":
            return (key, new_busy, q_depth_new, new_rbusy, rsu_load_new,
                    soc_new, a, k8s_new), (ys or None)
        next_carry = (key, new_busy, q_depth_new, new_rbusy, rsu_load_new, soc_new, a)
        if STATE_STUDY:
            next_carry += ((rb, step_index + jnp.int32(1)),)
        if ROUND_ROBIN:
            next_carry += (rr_pointer,)
        if P2C:
            next_carry += (p2c_tick + jnp.int32(1),)
    ...
    if STATE_STUDY:
        out["rsu_state_delay"] = {
            "schema_version": "model_c_placement_workload_delay_v1",
            "requested_ms": STATE_DELAY,
            "clock": "one_second_admission_batches_continuous_service_between_batches",
            "snapshot_phase": "previous_batch_post_admission_then_elapsed_service",
            "delayed_fields": ["placement_workload_ms"],
            "admission_state": "live_selected_rsu_workload_and_load",
            "reservations": "current_batch_admitted_work_only_immediate_ack",
            "warmup": "first_batch_live_state_actual_age_zero",
            "warmup_steps": min(T, 1) if STATE_DELAY else 0,
            "timestamp_origin": "elapsed_ms_from_first_trace_row",
        }
    out["extension_schema"] = "e3a_csf3_v1_records"
    if P2C:
        out["p2c_contract"] = {
            "counter": "SplitMix64_exact_uint64_via_uint32_limbs",
            "ordinal": "task_slot * padded_fleet_width + vehicle_slot",
            "state_age_ms": 0,
            "compute_policy": "fixed_1x",
            "feasibility": "current raw backlog < deadline and current occupancy < cap",
            "n0_precedence": ["v2i_unavailable", "v2i_gate_rejected", "v2i_cap_rejected"],
            "reservation": "immediate causal float32, carried across task slots",
            "modulo_mapping_is_not_exact_uniform": True,
        }
    ...
    if PERSTEP or PERTASK:
        ...
        if PERSTEP:
            ...
            if STATE_STUDY:
                step_keys.extend([
                    "state_observed_at_ms", "state_captured_at_ms",
                    "state_actual_age_ms", "state_warmup", "rsu_report_busy_ms",
                    "rsu_start_busy_ms", "rsu_pre_drain_busy_ms"])
        ...
        if PERTASK:
            ...
            np.savez_compressed(args.per_task_out,
                                **({name: ys_np[name] for name in P2C_TASK_KEYS}
                                   if P2C else {}),
                                ...
                                **({"task_placement_busy_ms": ys_np["task_placement_busy_ms"]}
                                   if STATE_STUDY else {}),
