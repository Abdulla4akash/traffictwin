#!/usr/bin/env python3
"""Twelve-cell, three-arm matched replication with a frozen analysis plan."""
import argparse
import csv
...
def verify_identity(m):
    ...
    for name, expected in m["campaign_files_sha256"].items():
        require(sha(ROOT/name) == expected, f"Campaign dependency changed: {name}")
    import jax
...
def command_for(m, cell, directory, steps):
    command = [m["python"], "-u", str(Path(m["code_checkout"]) / "eval/eval_sumo_stage1_mc.py"),
               ...
               "--fleet-seed", str(cell["fleet_seed"]), "--rsu-cap-abs", str(m["rsu_capacity_tasks"]),
...
def validate(directory, m, cell, steps, reference=None):
    ...
    require(s["fleet_seed"] == cell["fleet_seed"] and s["fleet"] == m["fleet"], "Fleet mismatch")
    ...
    workload_checks.validate(directory, m, cell, steps)
...
def main():
    ...
    if (ROOT/"status.json").exists():
        require(args.resume, "Campaign already started; use --resume")
        previous = json.loads((ROOT/"status.json").read_text())
        require(previous["manifest_sha256"] == sha(MANIFEST), "Manifest changed since launch")
    ...
    verify_identity(m)
    child = None
    status = {"status":"starting", "pid":os.getpid(), "started_at":now(), "manifest_sha256":sha(MANIFEST),
              "full_cells_total":len(m["cells"]), "full_cells_completed":[], "preflight_completed":[]}
    write_json(ROOT/"status.json",status)

    def run(cell, phase, steps, reference=None):
        nonlocal child
        reference = Path(reference) if reference is not None else None
        parent = ROOT/phase/cell["id"]
            ...
            for attempt in sorted(parent.glob("attempt_*")):
                ...
                if path.exists():
                    ...
                    if receipt["status"] == "passed":
                        require(all(sha(attempt/name)==value for name,value in receipt["sha256"].items()), "Completed output changed")
                        return attempt
        directory = parent/f"attempt_{len(list(parent.glob('attempt_*')))+1:03d}"
        directory.mkdir()
        verify_identity(m)
        require(shutil.disk_usage(ROOT).free >= m["minimum_free_bytes"], "Insufficient disk space")
        ...
        print(f"{now()} Starting {phase} {cell['id']} ({steps} steps)",flush=True)
        ...
        with (directory/"stdout.log").open("w") as out, (directory/"stderr.log").open("w") as err:
            ...
            status.update(status="running",phase=phase,current=cell["id"],fleet_seed=cell["fleet_seed"],
                          current_output=str(directory),cell_pid=child.pid,cell_started_at=now(),steps=steps)
            while child.poll() is None:
                status.update(updated_at=now(),cell_elapsed_s=round(time.monotonic()-start,1))
                write_json(ROOT/"status.json",status)
                require(time.monotonic()-start < m["max_cell_wall_seconds"], "Cell time limit exceeded")
                require(shutil.disk_usage(ROOT).free >= m["minimum_free_bytes"], "Disk space fell below stopping threshold")
                time.sleep(10)
        code = child.returncode
        child = None
        require(code == 0, f"Evaluator failed with code {code}; see {directory/'stderr.log'}")
        status.update(status="validating",cell_pid=None,updated_at=now())
        write_json(ROOT/"status.json",status)
        verify_identity(m)
        validate(directory,m,cell,steps,reference)
        print(f"{now()} Validated {phase} {cell['id']}",flush=True)
        return directory
    ...
    try:
        for cell in m["cells"]:
            reference = smokes.get(f"seed_{cell['fleet_seed']}_per_task")
            directory = run(cell,"preflight",m["preflight_steps"],reference)
            smokes[cell["id"]] = directory
            status["preflight_completed"] = list(smokes)
            write_json(ROOT/"status.json",status)
        print(f"{now()} All twelve preflight probes passed; starting full replication",flush=True)
        for cell in m["cells"]:
            reference = completed.get(f"seed_{cell['fleet_seed']}_per_task")
            directory = run(cell,"cells",m["steps"],reference)
            with np.load(smokes[cell["id"]]/"per_task.npz") as short, np.load(directory/"per_task.npz") as full:
                for field in ("task_type","task_active","task_outcome","task_met","task_ingress_rsu",
                              "task_selected_execution_rsu","task_execution_rsu","task_lat_ms"):
                    require(np.array_equal(full[field][:m["preflight_steps"]],short[field]), f"Full-run prefix differs: {field}")
            completed[cell["id"]] = str(directory)
            status["full_cells_completed"] = list(completed)
            write_json(ROOT/"status.json",status)
        status.update(status="analyzing",current=None,phase="analysis",updated_at=now())
        write_json(ROOT/"status.json",status)
        verify_identity(m)
        import analysis
        analysis.analyze(ROOT,m,completed)
        status.update(status="completed",finished_at=now(),report=str(ROOT/"RESULTS.md"),outputs=completed)
        ...
        return 1
    write_json(ROOT/"status.json",status)
    print(f"{now()} Completed all twelve cells and primary/supplementary analyses",flush=True)
    return 0
