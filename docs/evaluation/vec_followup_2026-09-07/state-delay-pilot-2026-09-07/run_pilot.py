#!/usr/bin/env python3
"""Serial, restartable-between-cells pilot; evaluator code stays immutable."""
import argparse
import csv
...
def command_for(m, cell, directory, steps):
    command = [m["python"], "-u", str(Path(m["code_checkout"]) / "eval/eval_sumo_stage1_mc.py"),
               ...
               "--max-steps", str(steps), "--seed", "0", "--fleet", "uk2030",
               "--fleet-seed", "1", "--rsu-cap-per-veh", "2.5",
...
def validate(directory, m, cell, steps, reference=None):
    ...
    require(s["T"] == steps and s["maxN"] == 2488, "Trace dimensions mismatch")
    require(s["fleet_seed"] == 1 and s["fleet"] == "uk2030", "Fleet mismatch")
    require(s["rsu_lb"] == cell["mode"], "Mode mismatch")
    require(s["rsu_max_concurrent"] == 6220 and s["rsu_backhaul_ms"] == 0,
    ...
    with np.load(directory / "per_step.npz") as step:
        ...
        require((load >= 0).all() and (load <= 6220).all() and (busy >= 0).all(), "Invalid live queue")
        if cell["delay_ms"] is not None:
            delay = cell["delay_ms"]
    ...
    with np.load(directory / "per_task.npz") as task:
        for name in task.files:
            a = task[name]
            require(a.shape == (steps, 5, 2488), f"Unexpected task shape: {name}")
        ...
        require(((execution[admitted] >= 0) & (execution[admitted] < 10)).all(), "Invalid execution RSU")
...
def analyze(completed):
    ...
    lines = ["# State-delay pilot results", "", "Single fleet draw (seed 1), Manchester incident, 3,600 simulated seconds per condition.", "", "| Condition | Offered-task deadline attainment | Change from fresh (pp) | Change from ingress (pp) |", "|---|---:|---:|---:|"]
    for row in rows:
    ...
    lines += ["", "This is a single-draw pilot, so these are descriptive comparisons without replicate-level confidence intervals.", "Placement sees delayed workload reports; admission and acknowledgements are current. Arrivals retain Model C's one-second batches. Equal reports during idle periods do not establish general resilience to communication delay.", "", "All cells passed their recorded validation. Full metrics are in comparison.csv and each cell's summary.json."]
...
def main():
    ...
    try:
        # The merged four delay modes have signed source-hash-bound ten-step
        # evidence already. Add the ingress smoke before any full pilot cell.
        run({"name":"ingress_smoke","mode":"ingress_dla","delay_ms":None},ROOT/"preflight",10)
