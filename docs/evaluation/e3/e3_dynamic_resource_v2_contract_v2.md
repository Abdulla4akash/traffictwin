# E3 Dynamic Resource v2 — Scientific Contract v2

**Campaign:** `e3-dynamic-resource-v2`
**Contract version:** `e3_dynamic_resource_v2_contract_v2`
...
JSON is the single normative scientific contract (`e3_dynamic_resource_v2_contract_v2.json`). Markdown is a deterministic generated view via `render_markdown(data)` and must byte-equal its output; byte mismatch is authoritative.
...
## 12b. Successor provenance (v2)

- successor_version: `v2`
- superseded_candidate_sha: `79abfe89f9eb55f490c9f578172256d7a13714a3`
- superseded_promoted_integration_sha: `bbd955ed181d2a82183158ded8ad86116809bf27`
- v1_json_sha256: `0d8c545703405c59dce5b1d1c7d38a63b31341c23364de43c6530862931fa160`
- v1_contract_path: `docs/evaluation/e3/e3_dynamic_resource_v2_contract_v1.json`
- exact_reasons: `waiting-room occupancy-transition omission and comparator rejection-taxonomy omission`
- reasons: waiting-room occupancy-transition omission; comparator rejection-taxonomy omission
- successor_authoring_branch: `worker/e3-lane-01-contract-v3`
- successor_prepared_base_sha: `bbd955ed181d2a82183158ded8ad86116809bf27`

---

## 12c. Waiting-room occupancy drain (v2 correction)

- per_rsu_once_per_1000ms_outer_tick_after_all_five_task_slots: true
- uses_true_pre_drain_state_never_stale_view: true
- B_pre = true pre-drain backlog_work_ms; L_pre = true pre-drain waiting-room task count
- drain_work_ms: `D = min(B_pre, active_capacity_units * 1000)`
- backlog_post: `B_post = B_pre - D`
- if B_post <= 0: tasks_drained = L_pre, L_post = 0
- else (B_post > 0): tasks_drained = floor(L_pre * D / max(B_pre, 1e-6)), L_post = max(L_pre - tasks_drained, 1)
- floor_term_is_tasks_removed_not_remaining_occupancy: true (floor term is tasks removed)
- dynamic_capacity_changes_work_drain_and_indirectly_released_occupancy: true; never_multiplies_waiting_room_ceiling 6220
- full_work_drain_forces_zero_occupancy: true; positive_residual_work_retains_at_least_one_task: true
- fixed_1x_must_reproduce_frozen_E2d_exactly: true
- simulated_count_transition_does_not_make_lifecycle_available: started/compute_completed/returned/dropped remain null
- pin_vectors:
  - B=4000 L=4 u=1 -> D=1000 B_post=3000 tasks_drained=1 L_post=3
  - B=500 L=4 u=3 -> D=500 B_post=0 tasks_drained=4 L_post=0
  - B=4000 L=1 u=1 -> D=1000 B_post=3000 tasks_drained=0 L_post=1

---

## 12d. Comparator rejection precedence (v2 correction)

- applies_to: `['ingress_dla', 'per_task_dla']`
- preserves_frozen_E2d_taxonomy_exactly: true; p2c_approved_n0_taxonomy_remains_separate_and_unchanged: true
- precedence:
  1. select comparator target;
  2. inactive/non-V2I yields no E3 V2I rejection class;
  3. current radio failure OR target already full at task-slot entry -> v2i_unavailable;
  4. otherwise strict observed-backlog gate failure -> v2i_gate_rejected;
  5. otherwise live in-slot cap failure caused by prior admitted reservations -> v2i_cap_rejected;
  6. otherwise admit;
  7. simultaneous gate and live cap failure is gate rejection;
  8. rejected work never changes true workload, occupancy, or overlay and execution is -1.
- P2C's approved n=0 taxonomy remains separate and unchanged; comparator precedence never replaces it.
- cross_strategy_comparability: total rejection/rejection share are comparable but individual V2I rejection classes are mechanism diagnostics and must not be presented as identically constructed cross-strategy contrasts

---

## 12e. Execution authority — HARD scientific execution hold (v2)
...
- e3a_authorized: `False` e3b_authorized: `False` e3c_authorized: `False`
...
- dormant_predeclaration_note: `The planned 56-cell design is retained only as a dormant predeclaration; planned cells are not execution authority and cannot run under this hold.`
- Note: The planned 56-cell design is retained only as a dormant predeclaration; planned cells are not execution authority and cannot run under this hold.
- Lane 08 may continue engineering plus minimal correctness-only smoke, but no Manchester trace comparative execution; Lanes 10-12 proceed with honest not-executed state and no placeholder results.
...
```json
{
  ...
  "comparator_rejection_precedence": {
    ...
    "cross_strategy_comparability": {
      "forbidden_presentation_is_identically_constructed_contrast": true,
      "individual_V2I_rejection_classes_are_mechanism_diagnostics": true,
      "must_not_be_presented_as_identically_constructed_cross_strategy_contrasts": true,
      "total_rejection_and_rejection_share_are_comparable_across_strategies": true
    },
    "details": {
      "cap_uses_true_current_plus_same_tick_not_stale": true,
      "execution_is_minus1_for_rejected": true,
      "gate_equality_is_gate_rejected": true,
      "radio_is_current_not_stale": true,
      "rejected_work_never_reserved_or_executed": true,
      "stale_occupancy_never_used_for_cap": true,
      "strict_gate_is_lt": true,
      "target_fullness_uses_current_true_occupancy_at_task_slot_entry": true,
      "target_fullness_uses_stale_free_occupancy": true,
      "v2i_cap_rejected_trigger": "live in-slot cap failure caused by prior admitted reservations (true_current_waiting_room_occupancy[rsu] + prior same-tick reservations >= queue_ceiling for target)",
      "v2i_gate_rejected_trigger": "strict observed-backlog gate failure (observed_decision_backlog_work_ms[rsu] >= task_deadline_ms)",
      "v2i_unavailable_trigger": "current radio failure OR target already full at task-slot entry"
    },
    "forbidden_behaviors": [
      ...
      "individual V2I rejection classes presented as identically constructed cross-strategy contrasts"
    ],
    "ordered_steps": [
    ...
    "p2c_approved_n0_taxonomy_remains_separate_and_unchanged": true,
    "p2c_separate": {
      "comparator_precedence_does_not_replace_p2c_taxonomy": true,
      "p2c_n0_taxonomy_unchanged": true,
      "p2c_uses_v2i_unavailable_v2i_gate_rejected_v2i_cap_rejected_for_n0_only": true
    },
    "precedence": {
      "step_1_select_comparator_target": true,
      "step_2_inactive_non_V2I_yields_no_E3_V2I_rejection_class": true,
      "step_3_current_radio_failure_OR_target_already_full_at_task_slot_entry_yields_v2i_unavailable": true,
      "step_4_otherwise_strict_observed_backlog_gate_failure_yields_v2i_gate_rejected": true,
      "step_5_otherwise_live_in_slot_cap_failure_caused_by_prior_admitted_reservations_yields_v2i_cap_rejected": true,
      "step_6_otherwise_admit": true,
      "step_7_simultaneous_gate_and_live_cap_failure_is_gate_rejection": true,
      "step_8_rejected_work_never_changes_true_workload_occupancy_or_overlay_and_execution_is_minus1": true
    },
  ...
  "successor_provenance": {
    "exact_reasons": "waiting-room occupancy-transition omission and comparator rejection-taxonomy omission",
    "note": "v1 remains historical; v2 is the successor binding the two missing scientific definitions",
    "reasons": [
  ...
  "waiting_room_occupancy_transition": {
    ...
    "floor_term_is_tasks_removed_not_remaining_occupancy": true,
    "forbidden_behaviors": [
    ...
    "never_multiplies_waiting_room_ceiling": true,
    "per_rsu_once_per_1000ms_outer_tick_after_all_five_task_slots": true,
    "pin_vectors": [
      {
        "B_post": 3000,
    ...
    "positive_residual_work_retains_at_least_one_task": true,
    "simulated_count_transition_does_not_make_lifecycle_available": true,
    "transition": {
      "else_B_post_gt_0": {
        "L_post": "max(L_pre - tasks_drained, 1)",
        "condition": "B_post > 0",
        "tasks_drained": "floor(L_pre * D / max(B_pre, 1e-6))"
      },
      "if_B_post_lte_0": {
        "L_post": 0,
        "condition": "B_post <= 0",
    ...
    "true_pre_drain_backlog_work_ms": "B_pre",
    "true_pre_drain_waiting_room_task_count": "L_pre",
    "uses_true_pre_drain_state_never_stale_view": true,
    "waiting_room_ceiling_never_scaled_with_compute_units": true,
    "waiting_room_ceiling_tasks_per_rsu": 6220
  }
}
```
