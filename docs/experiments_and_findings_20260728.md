...
| 9k | **Ceiling-law prediction test** (pre-registered; 12 cells, `inc`, seeds {60,61,62}, arms 0.5/0.25/0.1; 18.4 h) | predicted ceilings 19,980 / 9,990 / 3,996 ms with a ±5% band and a HELD/REFUTED/BOUNDED rule, all frozen before any cell ran, verdict code committed at 2/12 cells | **HELD — 27 of 27 pairs inside the band, none outside.** The law was fitted over a 3.3× range and these arms sit **7.5× below its floor and 25× below its baseline**; mean observed K is 40,157 / 40,309 / 40,716 against a fitted 39,959, and the in-range reference arm reproduces at 39,878–40,146 on three fresh seeds. **It sags systematically rather than failing**: error grows monotonically as capacity falls (T2: +1.08% → +1.73% → **+3.42%**), T2 carries the largest error in all nine arm×seed combinations, and seeds order 60>61>62 in every row — so the predeclared **BOUNDED** boundary sits just below the tested range. Secondaries: partition identical on all three seeds; attainment finally moved, **upward on all three seeds** as predeclared; p50 a **partial miss** (44.6–44.9 ms on seeds 60/62 but 45.7–46.2 on seed 61). Scope caveat: per row 9i the ceiling is an **RSU-queue** property, so this is a confirmed law about one subsystem exercised by ~40% of the fleet. [Record](evaluation/ceiling_law_prediction_results_20260729.md) |
| 9l | **Actor crossover** (predeclared exploratory comparison; `inc`, 2 actors × 4 capacities × 3 seeds, with the admitted pilot reused rather than rerun) | winner reversal at cap-2.5 vs cap-0.75; prediction (3) fixed as relative latency-slope contrast ≤5% | **No crossover, but prediction (3) REFUTED.** `ukfleettrain` wins at all four capacities by a stable 6.0835–6.0946 pp completion margin. Yet latency slopes are **3,828.2 vs 6,555.4 ms/capacity-unit**, a **52.53%** relative contrast against the frozen 5% band: the ceiling response is not actor-invariant under this design. The `uk2030` preset matches only the trained actor, so checkpoint policy and preset mismatch remain inseparable; never an algorithm-family or causal claim. [Record](evaluation/actor_crossover_results_20260730.md) |
| 9m | **Normal-regime onset-scaling prediction** (predeclared exact-identity rule; 36 fresh cells across `we`/`wd_pm`/`wd_am`, seeds {60–62}) | test `c_onset ∝ padded slots` from the `ev` onset bracket, six sharp predictions | **REFUTED — 4/6 sharp predictions correct; measured ordering also fails.** The load-bearing `wd_am`-at-0.1 prediction held, but `we` and `wd_pm` each bind at cap-0.25 under the frozen exact-equality rule. Crucial magnitude caveat: each failure is latency-only in one seed, just **0.000243 ms / 0.000024 ms**; completion is exactly unchanged. At cap-0.1 all four normal traces move in the same faint direction (completion +0.0011–0.0043 pp, latency −0.024–0.205 ms) with decisions unchanged. This refutes the exact rule, not evidence that density is irrelevant. [Record](evaluation/onset_scaling_prediction_results_20260730.md) |
...
*The law's pre-registered extrapolation test 7.5× below its fitted floor **held 27/27** within
the frozen ±5% band, while a later actor comparison refuted actor-independence of the latency
slope (52.53% contrast). The standing scope is therefore one trained actor's RSU-queue response,
not a universal property of the control.*
...
| 21 | **B-BUS Sparse-64 dawn→peak** (owner approved; whole fleet, outside VEC-06) | **GPU compute completed 5/5; retained archive passed local integrity/recomputation review, but the one-shot held-out execution deviated and the result remains non-admitted.** Retained cap-0.75 peak completion **0.519215 mean** (SD 0.088806; range 0.427187–0.618675), T1/T2/T3 0.337793/0.595472/0.545997; cap-2.5 0.519108. All seeds reached update 1,562 through checkpoint recovery. After the first return, `launchd` relaunched the supervisor and repeated fixed peak evaluation/packaging **147 times**. Training, actors and checkpoints did not change and no metric selection occurred, but the review retained and bound only the final returned artifact, so cross-return identity is unverifiable and the literal peak-once rule failed. Coverage remains **45.01% dawn / 46.00% peak**. [Homecoming result](evaluation/bbus_sparse64_homecoming_results_20260730.md) · [evidence](integration/evidence/bbus_sparse64_gpu_homecoming_20260730.json) · [checkpoint execution](evaluation/bbus_sparse64_checkpointed_execution_20260729.md) |
| 21b | **B-BUS Sparse-64 full fresh rerun** (owner approved; unchanged frozen pack) | **GPU compute completed 5/5 and the retained archive passed independent local review; retained cap-0.75 completion is 0.501355** (SD 0.088677; T1/T2/T3 0.314816/0.592028/0.521530), with cap-2.5 0.501233. Four apparent hourly runtime losses were traced to a stale one-hour CLI proxy credential, not demonstrated GPU failure, and recovered from checkpoints. A terminal-state ordering bug caused **one unintended second fixed held-out evaluation/return** after the first valid download. Training, actors, checkpoints and settings were not overwritten or changed, and no metric selection occurred; cross-return identity remains unverifiable. On 2 August the owner admitted the retained rerun as descriptive evidence with the immutable `admitted_with_execution_deviation` qualifier. It remains outside VEC-06, unpooled with protocol-confirmed VEC evidence and without actor-admission, causal, physical-completion or supervisor-approved standing. No session remains active. [Fresh-rerun record](evaluation/bbus_sparse64_clean_rerun_results_20260730.md) · [evidence](integration/evidence/bbus_sparse64_clean_rerun_gpu_homecoming_20260730.json) · [owner admission](evaluation/bbus_sparse64_clean_rerun_owner_admission_20260802.json) |
...
verified. Both GPU arms returned; the corridor remains preliminary/non-admitted. The earlier
Sparse-64 return remains execution-deviated/non-admitted after 147 unintended fixed peak
evaluations. The clean rerun is locally integrity-verified and owner-admitted only as descriptive
evidence with `admitted_with_execution_deviation` after one unintended second fixed held-out
evaluation/return. Its training, actors and checkpoints were not overwritten; neither Sparse-64
result is pooled with protocol-confirmed VEC evidence. The fresh rerun's terminal-state ordering
defect is repaired and tested. No browser extension or open browser tab is required, the
supervisor service is stopped, and no Colab session remains active.
