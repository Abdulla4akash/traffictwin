...
| 9c | **Per-RSU load asymmetry** (analysis-only) | 10 RSUs, 2 arms | **3 of 10 RSUs carry exactly zero load** at every capacity, a fourth under 5%, busiest ~24%; Gini 0.486→0.467 — the squeeze does not redistribute. **Interpretation revised by row 9j: it is not placement.** [Record](evaluation/rsu_load_asymmetry_20260728.md) |
...
| 9j | **RSU association analysis** (analysis-only, 3 cells; uses `veh_best_rsu`, a field nothing had touched) | why RSUs sit idle: placement vs tier partition vs association rule | **Most deployed RSU capacity is unusable because the association rule is load-blind, not because of placement.** Four RSUs run at **95.8–97.9%** of the concurrency bound while four sit at **0.0%**; the saturated four carry **99.1%** of load. The idle ones are neither out of range nor full — RSU 3 is selected as best **more often than RSUs 1, 2 or 6** and carries 2.8% of their load, and RSU 8 takes 22,939 `v2i` sends with a load of exactly 0. The tier-partition hypothesis was tested and rejected (tier-0 selection share is 0.375–0.435 across all ten, r=+0.31 with load). Mechanism read from producer source: `best_rsu_idx = jnp.argmax(all_v2i_q)` — link quality only, no load term, while `best_rsu_load_frac` is computed for the *observation* and never consulted by the association. The same four saturate across a 25× squeeze and a different seed, which is why the earlier analysis saw no redistribution. Consequence: the remedy is a software rule, not civil engineering, and every capacity number in this project describes **four** RSUs rather than ten. [Record](evaluation/rsu_association_analysis_20260729.md) |
