...
| 21 | **B-BUS Sparse-64 dawn→peak** (owner approved; whole fleet, outside VEC-06) | **GPU compute completed 5/5; retained archive passed local integrity/recomputation review, but the one-shot held-out execution deviated and the result is non-admitted.** Retained cap-0.75 peak completion **0.519215 mean** (SD 0.088806; range 0.427187–0.618675), T1/T2/T3 0.337793/0.595472/0.545997; cap-2.5 0.519108. All seeds reached update 1,562 through checkpoint recovery. After the first return, `launchd` relaunched the supervisor and repeated fixed peak evaluation/packaging **147 times**; actors/settings did not change and no metric selection occurred, but prior archives were overwritten, so repeat identity is unverifiable and the literal peak-once rule failed. Coverage remains **45.01% dawn / 46.00% peak**. [Homecoming result](evaluation/bbus_sparse64_homecoming_results_20260730.md) · [evidence](integration/evidence/bbus_sparse64_gpu_homecoming_20260730.json) · [checkpoint execution](evaluation/bbus_sparse64_checkpointed_execution_20260729.md) |
| 21b | **B-BUS Sparse-64 full fresh rerun** (owner approved; unchanged frozen pack) | **GPU compute completed 5/5 and the retained archive passed independent local review; retained cap-0.75 completion is 0.501355** (SD 0.088677; T1/T2/T3 0.314816/0.592028/0.521530), with cap-2.5 0.501233. Four apparent hourly runtime losses were traced to a stale one-hour CLI proxy credential, not demonstrated GPU failure, and recovered from checkpoints. A terminal-state ordering bug then caused **one unintended second archive return** after the first valid download; earlier archive overwritten, settings unchanged, no metric selection. The literal peak-once rule again failed, so the rerun is execution-deviated, descriptive and non-admitted. No session remains active. [Fresh-rerun record](evaluation/bbus_sparse64_clean_rerun_results_20260730.md) · [evidence](integration/evidence/bbus_sparse64_clean_rerun_gpu_homecoming_20260730.json) |
...
verified. Both GPU arms returned; the corridor remains preliminary/non-admitted, while both
Sparse-64 returns are locally integrity-verified but execution-deviated/non-admitted because
fixed peak evaluation repeated after first return. The fresh rerun's terminal-state ordering
