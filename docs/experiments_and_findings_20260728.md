...
| 21 | **B-BUS Sparse-64 dawn→peak** (owner approved; whole fleet, outside VEC-06) | **GPU execution interrupted; no scientific result.** The one dawn-weighted 64-site array still covers **45.01% dawn / 46.00% peak** vehicle-seconds. Five concurrent seed jobs completed only 350–354/1,562 updates before the Colab session terminated at 02:53 BST; no actor, held-out evaluation, summary or archive returned. A frozen-protocol rerun remains required. [Detailed settings and execution status](evaluation/bbus_dawn_peak_settings_and_preliminary_results_20260729.md) |
