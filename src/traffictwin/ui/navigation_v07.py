...
V07_PAGE_SPECS: tuple[V07PageSpec, ...] = (
    ...
    V07PageSpec(
        ...
        "Build & run",
        "app_pages/metric_contract_registry.py",
        "metric-contracts",
        ":material/contract:",
    ),
    V07PageSpec(
        ...
        "Build & run",
        "app_pages/contract_drafting.py",
        "contract-drafting",
    ...
    V07PageSpec(
        ...
        "Build & run",
        "app_pages/event_scenario_bridge.py",
        "event-scenario-bridge",
        ":material/alt_route:",
    ),
    V07PageSpec(
        UiPage.RUN_OVERVIEW,
    ...
    V07PageSpec(
        ...
        "Compare & test",
        "app_pages/calibration_workbench.py",
        "calibration",
        ":material/tune:",
    ),
    V07PageSpec(
        ...
        "Compare & test",
        "app_pages/tradeoff_explorer.py",
        "tradeoff-explorer",
        ":material/compare_arrows:",
    ),
    V07PageSpec(
    ...
    V07PageSpec(
        ...
        "Evidence & reports",
        "app_pages/evidence_admission.py",
        "evidence-admission",
        ":material/fact_check:",
    ),
    V07PageSpec(
        ...
        "Evidence & reports",
        "app_pages/baseline_registry.py",
        "baseline-registry",
        ":material/flag:",
    ),
    V07PageSpec(
        ...
        "Evidence & reports",
        "app_pages/reproducibility_replay.py",
    ...
    V07PageSpec(
        ...
        "Advanced",
        "app_pages/workspace_activation.py",
...
def validate_v07_page_specs(base: Path | None = None) -> None:
    ...
    # Expansion V1 additive specs — Lane 16 composition (required; fail visibly if broken)
    from traffictwin.ui.expansion_routes import (
        EXPANSION_PAGE_SPECS as _expansion_specs,  # noqa: N811
    )

    if len(_expansion_specs) != 4:
        raise ValueError(
...
def v07_navigation_pages() -> dict[str, list[object]]:
    ...
    for group in V07_NAVIGATION_GROUPS:
        ...
        # Expansion V1 composition — additive, coherent grouping (Lane 16) — required
        from traffictwin.ui.expansion_routes import (
            EXPANSION_PAGE_SPECS as _expansion_specs_nav,  # noqa: N811
        )

        if len(_expansion_specs_nav) != 4:
            raise ValueError(
                f"expansion routes must contain exactly 4 specs, got {len(_expansion_specs_nav)}"
            )
