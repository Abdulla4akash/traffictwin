# ruff: noqa: S603
"""Resumable full-scale B-DOMAIN MAPPO campaign for a disposable G4 VM."""
...
CAMPAIGN_VERSION = "bdomain-full-training-v1"
MODEL_SEEDS = (400, 401, 402, 403, 404)
...
PREDECLARATION_RELATIVE_PATH = "docs/evaluation/bdomain_training_predeclaration_20260728.md"
...
class Job:
    ...
    def identity(self) -> str:
        return f"train-{self.training_domain}__model-seed-{self.model_seed}"


def campaign_jobs() -> tuple[Job, ...]:
    return tuple(
        Job(domain, TASK_PROBABILITIES[domain], 0.7, False, 19, seed)
...
def verify_approval(repo_root: Path, receipt_path: Path) -> dict[str, Any]:
    ...
    if receipt.get("approved") is not True or receipt.get("held_out_authorised") is not False:
        raise ValueError("approval is not the exploratory B-DOMAIN authorization")
    if receipt.get("producer_data_authorised_for_colab") is not False:
        raise ValueError("B-DOMAIN must remain producer-data-free")
...
def _job_files(output_root: Path, job: Job) -> dict[str, Path]:
    ...
    return {
        ...
        "matrix_json": Path(f"{base}_domain_matrix_eval.json"),
        "matrix_npz": Path(f"{base}_domain_matrix_actions.npz"),
        "eval_log": Path(f"{base}_domain_matrix_eval.log"),
...
def _validate_completed_job(output_root: Path, job: Job) -> dict[str, Any]:
    ...
    matrix = json.loads(paths["matrix_json"].read_text())
    if matrix.get("scientific_evidence") is not False:
        raise ValueError(f"{job.identity}: malformed diagnostic status")
    if matrix.get("training_domain") != job.training_domain:
        raise ValueError(f"{job.identity}: training-domain mismatch")
    domains = matrix.get("evaluation_domains", [])
    if [domain.get("task_domain") for domain in domains] != list(TASK_DOMAINS):
        raise ValueError(f"{job.identity}: evaluation-domain order mismatch")
    for domain in domains:
        if domain["task_probabilities"] != list(TASK_PROBABILITIES[domain["task_domain"]]):
            raise ValueError(f"{job.identity}: task probabilities mismatch")
        if [level["capacity_per_slot"] for level in domain["levels"]] != list(CAPACITY_PER_SLOT):
            raise ValueError(f"{job.identity}: capacity order mismatch")
        if any(level["episodes"] != EVALUATION_EPISODES for level in domain["levels"]):
            raise ValueError(f"{job.identity}: episode count mismatch")
    with np.load(paths["matrix_npz"], allow_pickle=False) as fixed:
        action_shape = list(fixed["actions"].shape)
    if action_shape != [3, 2, EVALUATION_EPISODES, 200, 20]:
        raise ValueError(f"{job.identity}: domain-matrix action tensor mismatch")
    return {
        "curve_rows": len(rows),
        "effective_timesteps": int(rows[-1]["env_step"]),
        "last_training_row": rows[-1],
        "actor_input_shape": actor_shape,
        "domain_matrix_action_shape": action_shape,
        "domain_matrix_diagnostic": matrix,
        "files": {
            key: {
                "path": path.name,
...
def _run_job(
    ...
    env["VEC_JAX_BASELINE_ALPHA"] = "0.7"
    train_command = [
        sys.executable,
    ...
    eval_command = [
        sys.executable,
        str(repo_root / "gpu/real_bdomain/evaluate_domain_matrix.py"),
        "--source-root",
        str(source_root),
        "--actor",
        str(paths["actor"]),
        ...
        "--output-npz",
        str(paths["matrix_npz"]),
    ]
    started = datetime.now(UTC).isoformat()
    start = time.monotonic()
    with paths["train_log"].open("w") as log:
    ...
    eval_env = dict(env)
    eval_env["PYTHONPATH"] = str(repo_root)
    with paths["eval_log"].open("w") as log:
    ...
    manifest = {
        ...
        "environment": {**common_env, "VEC_JAX_BASELINE_ALPHA": "0.7"},
...
def run_campaign(
    ...
    transformation = json.loads((source_root / "bcap_source_transformation.json").read_text())
    ...
    design = {
        ...
        "learning_rate": 3e-3,
        "domain_matrix_diagnostic": {
            "training_domains": list(TASK_DOMAINS),
            "evaluation_domains": list(TASK_DOMAINS),
            "task_probabilities": {
                domain: list(probabilities)
                for domain, probabilities in TASK_PROBABILITIES.items()
                if domain in TASK_DOMAINS
            },
            "capacity_levels": list(CAPACITY_PER_SLOT),
            "episodes_per_cell": EVALUATION_EPISODES,
        },
