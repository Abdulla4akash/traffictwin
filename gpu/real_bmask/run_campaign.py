# ruff: noqa: S603
"""Resumable full-scale B-MASK MAPPO campaign for a disposable G4 VM."""
...
CAMPAIGN_VERSION = "bmask-full-training-v1"
MODEL_SEEDS = (300, 301, 302, 303, 304)
TREATMENTS = (("capacity_aware_unmasked", False), ("capacity_aware_masked", True))
...
PREDECLARATION_RELATIVE_PATH = "docs/evaluation/bmask_training_predeclaration_20260728.md"
...
def campaign_jobs() -> tuple[Job, ...]:
    return tuple(
        Job(treatment, use_mask, 0.7, 19, seed)
        for treatment, use_mask in TREATMENTS
...
def verify_approval(repo_root: Path, receipt_path: Path) -> dict[str, Any]:
    ...
    if receipt.get("approved") is not True or receipt.get("held_out_authorised") is not False:
        raise ValueError("approval is not the exploratory B-MASK authorization")
    if receipt.get("producer_data_authorised_for_colab") is not False:
        raise ValueError("B-MASK must remain producer-data-free")
...
def _job_files(output_root: Path, job: Job) -> dict[str, Path]:
    ...
    return {
        ...
        "grid_json": Path(f"{base}_mask_grid_eval.json"),
        "grid_npz": Path(f"{base}_mask_grid_actions.npz"),
        "eval_log": Path(f"{base}_mask_grid_eval.log"),
...
def _validate_completed_job(output_root: Path, job: Job) -> dict[str, Any]:
    ...
    grid = json.loads(paths["grid_json"].read_text())
    if grid.get("scientific_evidence") is not False:
        raise ValueError(f"{job.identity}: malformed diagnostic status")
    modes = grid.get("evaluation_modes", [])
    if [mode.get("mode") for mode in modes] != list(EVALUATION_MODES):
        raise ValueError(f"{job.identity}: evaluation-mode order mismatch")
    for mode in modes:
        if [level["capacity_per_slot"] for level in mode["levels"]] != list(CAPACITY_PER_SLOT):
            raise ValueError(f"{job.identity}: capacity order mismatch")
        if any(level["episodes"] != EVALUATION_EPISODES for level in mode["levels"]):
            raise ValueError(f"{job.identity}: fixed-grid episode count mismatch")
    if any(level["selected_infeasible_rate"] != 0.0 for level in modes[1]["levels"]):
        raise ValueError(f"{job.identity}: masked evaluation selected an infeasible action")
    with np.load(paths["grid_npz"], allow_pickle=False) as fixed:
        action_shape = list(fixed["actions"].shape)
    if action_shape != [2, 4, EVALUATION_EPISODES, 200, 20]:
        raise ValueError(f"{job.identity}: mask-grid action tensor mismatch")
    return {
        "curve_rows": len(rows),
        "effective_timesteps": int(rows[-1]["env_step"]),
        "last_training_row": rows[-1],
        "actor_input_shape": actor_shape,
        "mask_grid_action_shape": action_shape,
        "mask_grid_diagnostic": grid,
        "files": {
            key: {
                "path": path.name,
...
def _run_job(
    ...
    if job.use_action_mask:
        train_command.append("--use-mask")
    eval_command = [
        sys.executable,
        str(repo_root / "gpu/real_bmask/evaluate_mask_grid.py"),
        "--source-root",
        str(source_root),
        "--actor",
        str(paths["actor"]),
